...
### 10.11. Primary congruence branch and 2-adic offset in the irregular branch

The centered halo template of Section `10.9` suggests that the family-dependent data in the irregular branch is not the halo geometry itself, but the placement of that geometry in the depth variable `d`. This placement is controlled by a distinguished primary branch.

On the lifting branch, write

```text
d = 2n.
```

Then Proposition `11.8.1.7.1` shows that the valuation condition
...
For the irregular families, this congruence still has a compatible distinguished solution branch. The difference from the regular branch is not the absence of a congruence tower, but the presence of a visible halo around it.

Accordingly, define the primary congruence branch of an irregular family to be the compatible sequence of solution classes
...
These classes give the primary spike locations at successive levels of `2`-adic resolution. In this sense, the true family-dependent offset of the irregular halo is not an arbitrary shift read off from a fixed modulus. It is the `2`-adic limit of the compatible lift branch

```text
d*_∞(ω) = lim_k d_k*(ω),
```

viewed as a `2`-adic quantity.

For `ω = 17`, the primary branch begins:
...
Thus the visible primary spikes are finite truncations of a single compatible `2`-adic offset branch.

This gives a sharper structural picture of the irregular families. They are not best described as having merely “shifted halo shells.” Rather, they appear to have the form

```text
primary congruence branch  +  universal centered dyadic halo template.
```

The primary congruence branch supplies the family-dependent placement of the template, while the halo geometry itself appears to be universal after recentering.

At present this remains an empirical structural interpretation. Its significance is that the irregular branch now appears to possess not only a common centered halo shape, but also a coherent candidate for the true family-dependent offset: the compatible congruence lift `d_k*(ω)`.

### 10.11A. Sharper 2-adic anchor picture for the irregular branch

The primary congruence branch of Section `10.11` suggests a further sharpening of the irregular picture. The family-dependent object does not appear to be merely a sequence of isolated spike locations

```text
d_k^*(ω),
```

but a single compatible `2`-adic offset whose finite truncations are the observed primary spikes.
...
Then Proposition `11.8.1.7.1` identifies the valuation condition
...
For the irregular families, this congruence appears to determine a compatible distinguished solution branch
...
The sharper interpretation is that these classes are finite truncations of a single `2`-adic anchor

```text
d_∞(ω) in 2 Z_2,
```

characterized informally by
...
Equivalently, on the even branch one may regard

```text
n_∞(ω) = d_∞(ω) / 2
```

as the `2`-adic exponent satisfying

```text
9^(n_∞(ω)) = ω^(-1)
```

in `Z_2`, with the observed branches `d_k^*(ω)` given by successive residue truncations of this `2`-adic exponent.

Under this interpretation, the irregular halo template of Sections `10.9`–`10.11` becomes more coherent. The family-dependent data is not the halo geometry itself, but the placement of a universal centered valuation law around the anchor `d_∞(ω)`.

More precisely, the computations suggest the following empirical law. For an irregular family and for even displacement
...
whenever `Δ` is measured off the primary spike and away from the spike itself. Thus the off-spike halo behaves exactly like the family `ω = 1`, but translated to the family-dependent anchor location.

In this form, the primary spike is no longer an isolated anomaly. It is the truncation point at which the centered `ω = 1` law breaks because the chosen depth has landed unusually close to the `2`-adic anchor `d_∞(ω)` itself.

This interpretation also gives a natural meaning to the spike height. Let

```text
A_k(ω) = v_2(ω · 3^(d_k^*(ω)) - 1)
```

denote the height of the primary spike at level `k`. Then the data suggest that this height is governed by the `2`-adic accuracy of the truncation:

```text
A_k(ω) = 2 + v_2(d_∞(ω) - d_k^*(ω)).
```

Equivalently, the spike height records how many `2`-adic digits of the full anchor have already been captured by the finite truncation `d_k^*(ω)`.

For the minimal irregular example `ω = 17`, the primary branch begins

```text
4, 12, 76, 588, 8780, 139852, ...
```

and the corresponding spike heights are

```text
5, 8, 11, 15, 19, 20, ...
```

The successive jumps in the branch are
...
That is, the spike height appears to be determined by the `2`-adic size of the next refinement jump in the primary branch.

Thus the irregular lifting families are provisionally understood not merely as
...
In this picture, the family-specific information enters through the anchor `d_∞(ω)` and through the height of approximation achieved by its current truncation, while the surrounding halo geometry is universal.

At present this remains an empirical structural interpretation rather than a proved theorem. Its significance is that it replaces the looser language of a “shifted halo” by a more rigid candidate mechanism: the irregular branch appears to be organized by a genuine `2`-adic offset, whose truncations produce the visible primary spike locations and whose neighborhood carries the transplanted `ω = 1` valuation law.
...
**Definition 11.8.1.8.1 (raw congruence level).** Let `ω` be odd, and let `k >= 1`. The level-`k` congruence set of `ω` is
...
##### 11.8.2.6. Route A implications

The tower map changes the interpretation of Stage 1 of Route A. Stage 1 is no longer merely the search for isolated closed formulas for

```text
s = v_2(3^d ω - 1).
```

It has now split into a geometric classification problem on the lifting branch.

The first-layer mod `8` classification still gives the coarse partition of the reduced state space. It identifies where lifting is possible and isolates the even branch

```text
ω ≡ 1 (mod 8),   d = 2n
```

as the main lifting regime. But once this branch is reached, the problem no longer appears to be uniform.

Instead, the current evidence suggests that Stage 1 now has two distinct lifting programs.

**(a) Regular lifting program.** In the regular families, the even branch is governed directly by the congruence tower
...
* to understand the existence and compatibility of the distinguished congruence tower,
* to determine how the exceptional lifting class refines through higher powers of `2`,
* and to convert that tower control into exact statements about valuation growth, `3`-gain, and downstream quantities such as `d_+`.

In this branch, the congruence tower itself appears to account for the full visible lifting geometry.

**(b) Irregular lifting program.** In the irregular families, the same congruence tower still appears to generate a primary branch, but it no longer exhausts the observed geometry. Instead, the main branch is surrounded by a centered dyadic halo template which persists after recentering at the primary spike.

Here the main tasks are:

* to understand the primary congruence branch `d*_k(ω)` as the true family-dependent offset,
* to describe the universal centered halo template and its dyadic refinement,
* and to determine how the parameter `ω` controls the placement and magnitude of that template.

Thus the irregular branch does not appear to lack structure. Rather, it appears to possess a richer structure of the form

```text id="h7hr7j"
primary congruence branch  +  universal centered dyadic halo template.
```

This leads to a revised formulation of Stage 1.

**Stage 1 now appears to require:**

1. proving the exact congruence reformulation on the even branch,
2. defining and controlling congruence towers as structural objects,
3. separating regular from irregular lifting geometries,
4. explaining the regular branch through its congruence tower alone,
5. and explaining the irregular branch through the interaction of a primary congruence branch with the centered dyadic halo template.

Accordingly, the aim of Stage 1 is no longer just to determine `s` on a few isolated families. It is to determine the tower geometry governing `s` on natural infinite families, and to identify the arithmetic invariants of `ω` that select the relevant tower type.

In this way, the tower map does not merely add new examples to Route A. It reorganizes the program itself: the valuation problem has become a problem of geometric lifting structure on the reduced state space.

#### 11.8.2.6A. Local anchor mechanism near a high spike

The empirical anchor picture of Sections `10.9`–`10.11A` suggests that the irregular branch is governed locally by a simple valuation mechanism once one centers at a sufficiently high primary spike. The point of the present subsection is to isolate that mechanism as the next exact theorem target of Route A.
...
Thus `H` is the height of the spike at the chosen depth `d_*`.

Now let `Δ` be an even displacement and write
...
Since `v_2(A_*) = H`, the second term has valuation at least `H`. Accordingly, whenever the first term has strictly smaller valuation, it dominates the sum.

For even `Δ != 0`, Proposition `11.8.1.1.1` gives
...
This is exactly the transplanted `ω = 1` halo law suggested empirically in Section `10.9`: away from the spike itself, the valuation is governed not by the full family data, but by the `2`-adic size of the displacement from the spike.

This motivates the following theorem target.

**Provisional Theorem Target.** Let `ω` be odd and let `d_*` be an even depth with

```text
H = v_2(ω·3^(d_*) - 1) >= 3.
```

Then for every even `Δ != 0` satisfying
...
Equivalently: once a spike of height `H` is present, the off-spike valuations in its `2`-adically visible neighborhood are forced to follow the same dyadic shell law as the family `ω = 1`, up to the point where the displacement valuation reaches the spike height.

This statement would explain the universal centered halo template of the irregular branch without yet requiring a full global description of the family-dependent anchor `d_∞(ω)`. It would also sharply separate the two layers of the irregular picture:

* the **family-dependent data**, namely the location `d_*` and height `H` of the spike,
* and the **universal local law** governing the surrounding halo once the family has been centered at that spike.

In this sense, the local anchor mechanism is the natural next exact step in Route A. It does not yet solve the full irregular branch, but it would convert a substantial part of the current empirical geometry into theorem-level structure.

#### 11.8.2.6B. Supporting lemmas for the local anchor mechanism

The provisional theorem target of `11.8.2.6A` reduces to two new local lemmas together with the already established even-displacement valuation law of Proposition `11.8.1.1.1`.

**Lemma 1 (spike decomposition).**  
Let `ω` be odd, let `d_*` be even, and define
...
Then for every integer `Δ`,

```text id="4x1i2m"
ω·3^(d_*+Δ) - 1 = (3^Δ - 1) + A_*·3^Δ.
```

In particular, if `Δ` is even and `Δ != 0`, then
...
The valuation statement follows immediately because multiplication by the odd integer `3^Δ` does not change `2`-adic valuation. ∎

**Lemma 2 (valuation dominance).**
...
Consequently, under the notation of Lemma 1, if `Δ` is even, `Δ != 0`, and
...
with `u,v` odd and `a < b`. Then

```text
X + Y = 2^a(u + 2^(b-a)v).
```

Since `b-a >= 1`, the term `2^(b-a)v` is even, so `u + 2^(b-a)v` is odd. Hence `v_2(X+Y)=a=v_2(X)`. Applying this to

```text
X = 3^Δ - 1,   Y = A_*·3^Δ
```

and using Lemma 1 gives the consequence. ∎

By Proposition `11.8.1.1.1`, for every even integer `Δ != 0` one has

```text id="nhj7da"
v_2(3^Δ - 1) = 2 + v_2(Δ).
```

Combining this established valuation law with Lemmas 1 and 2 yields the local anchor law.

**Corollary (local anchor law near a high spike).**
Let `ω` be odd, let `d_*` be even, and suppose
...
**Proof.** By Proposition `11.8.1.1.1`,

```text
v_2(3^Δ - 1) = 2 + v_2(Δ).
```

Under the hypothesis `2 + v_2(Δ) < H`, this gives

```text
v_2(3^Δ - 1) < H = v_2(A_*·3^Δ).
```

Lemma 2 therefore applies to the decomposition of Lemma 1 and yields

```text
v_2(ω·3^(d_*+Δ) - 1) = v_2(3^Δ - 1) = 2 + v_2(Δ).
```

∎

This corollary formalizes the local mechanism behind the empirical centered halo: once a spike of height `H` is present, all even displacements whose shell valuation remains strictly below `H` are forced to follow the universal dyadic law inherited from the family `ω = 1`.
...
These results do not depend on the empirical tower classifications. They identify the exact congruence object that must be studied on the even branch.
...
- the universal centered dyadic halo template in the irregular branch,
- and the interpretation of the irregular family-dependent offset through the primary congruence branch `d*_k(ω)`.

Thus the irregular branch should no longer be regarded merely as a repository of unresolved exceptions. It already exhibits a coherent empirical geometry.
...
- and any complete description of how `ω` determines the primary spike offset and valuation magnitude in the irregular branch.
...
empirical but strongly supported:
    ...
    universal centered dyadic halo template,
    primary congruence branch as the irregular offset

conjectural:
    full arithmetic classifier of tower type,
    global dependence of offset and magnitude on ω
```

This distinction is important for the discipline of Route A. The current synthesis map is already strong enough to guide the next stages of investigation, but it should not yet be read as a completed theorem of the reduced dynamics. It is a partly formal, partly empirical structure whose outstanding task is to convert the coherent observed geometries into proved mechanisms.
