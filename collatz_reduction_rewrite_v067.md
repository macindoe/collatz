...
should still be treated only as a heuristic classifier at this stage. The later logarithmic identification of the anchor (`11.8.3.6`) supplies a structural reason for caution beyond small sample size: tower type is a property of the `2`-adic digits of `-log ω / log 9`, and control of those digits by the residue of `ω (mod 3)` would be surprising; see `11.8.4.3(c)`.
...
The current data still suggests a distinction between regular and irregular visible tower geometries. But no full theorem yet separates these tower types globally, and the logarithmic identification of the anchor (Theorem `11.8.3.6.6`) recalibrates what should be expected here.

Since `N(ω) = -log ω / log 9`, the tower type of a family is a statement about the `2`-adic digit expansion of the logarithm of a small integer. Describing digit patterns of `p`-adic logarithms of integers is transcendence-theory territory: such digits are expected to behave pseudo-randomly, and no mechanism is known by which a fixed small modulus could govern them. Two consequences follow.

First, the open question is expected to be genuinely hard, and its difficulty is now correctly attributed: it is not a gap in the Collatz-specific machinery but an instance of a classical hard problem.

Second, the previously proposed classifier
...
is demoted from this ledger. A mod-`3` law for the digits of `log ω` would be surprising, and the supporting sample (four regular, four irregular families) is far too small to outweigh that prior. The proposal is retained only as a heuristic observation in Appendix `A.4.6`, pending substantially more computation; it should not currently be treated as a theorem target.
...
The `3`-gain law is therefore closed as a formal theorem on the entire lifting branch. What remains open on the anchor side is expected to remain open: any further "arithmetic organization" of the digits of `N(ω)` for irregular families would constitute progress on the digit behavior of `2`-adic logarithms, a problem independent of, and older than, the present program.
...
This proposed split should be regarded as empirical only, and the main text now records a structural reason for skepticism (see `11.8.4.3(c)`): by the logarithmic identification of Theorem `11.8.3.6.6`, tower type is a property of the `2`-adic digits of `-log ω / log 9`, and a mod-`3` law governing digits of a `2`-adic logarithm would be surprising. With only four families on each side of the split, the observation is retained here as a historical heuristic, demoted from the main-text ledger, and flagged as requiring substantially more computation before it could re-enter as a theorem target.
...
inside the valid lifting branch `ω ≡ 1 (mod 8)`. (See `11.8.4.3(c)` and `A.4.6` for why this association is nonetheless treated with caution: it proposes mod-`3` control over the digits of a `2`-adic logarithm, and three regular examples carry little evidential weight against that prior.)
