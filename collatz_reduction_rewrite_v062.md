...
The first part of the conversion from valuation synthesis to reduced dynamics is now complete. Section `11.8.5` proves the exact `3`-gain law for all families on the lifting branch
...
```text
on the lifting branch, 3-gain is controlled by the parity of v_2(n - N(ω)).
```
...
and anchor displacement

```text
ε = n - N(ω),
```

the next odd seed gains a factor of `3` exactly when
...
This is a genuine reduction of the dynamics. The classification does not depend on whether the visible tower geometry is regular or irregular; once the family anchor `N(ω)` is fixed, the `3`-gain pattern is determined by dyadic shell parity around that anchor.
...
Theorem `11.8.5.2.1` applies to all families on the lifting branch as defined in `11.8.1.8.2`: odd `ω ≡ 1 (mod 8)` with `d` even. It does not cover the other residue classes of the mod `8` classification, which are handled at the first layer by Proposition `11.8.1.4.1` and do not reach the lifting regime. Within the lifting branch, the distinction between regular and irregular tower types does not affect the `3`-gain criterion itself.
...
The `3`-gain law is therefore closed as a formal theorem. The remaining open question is an anchor-side one: whether the `2`-adic expansion of `N(ω)` for irregular families has further arithmetic organization beyond what the spike sequence currently reveals.
