...
**Equivalent names.** The mirror side also names this object as boundedness versus escape of the positive realization height `(H_{p,q})` (reverse.md 14.15.4.5, the equivalence theorem; 14.15.5.4, the combined three-way characterization; 14.15.9, the whole-period form). The equivalence is stated and owned there; this page only lists it among the object's equivalent forms.
...
- **The seam identity.** The cycle-side transport recurrence (cycles.md Remark 12.6.1.1) and the mirror-side rotation lemma (reverse.md 14.15.9.2) are the same fact under the seam identity — the two escapes' cycle bookkeeping meets on one identity.
