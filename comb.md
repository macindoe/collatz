...
## 18.9. Branching signatures

The author's question of 2026-09-17: the point of cataloguing the comb is to find deterministic behaviour that groups reduced states; is there a way to search for patterns in how the comb branches? This section proves that a node's subtree shape to any finite depth is a function of its depth `D` and its core's residue `Ω mod 3^k` for a computable `k` (18.9.1–18.9.3), then searches for any grouping *finer* than residue classes that predicts that shape (18.9.4). Grouping here is by residue class and by signature, two different things kept apart throughout; no family-frame vocabulary is used.

### 18.9.1. The level-1 branching laws

**Lemma 18.9.1.** For a node `(Ω,D)` with doors `y_a = 2^{D−a}3^aΩ − 1`, `0 ≤ a < D`, the depths of its children (Proposition 18.2.3) are:

- **cascade child**: `1` if `D ≥ 2`; `1 + v₃(4Ω − 1)` if `D = 1`;
- **door child through door `a ≥ 2`** (always live, Theorem 14.5.1): depth exactly `1`;
- **door child through door `a = 1`** (live iff `D ≥ 2`): depth `1 + v₃(2^{D+1}Ω − 1)`;
- **door child through the top door `a = 0`** (live iff `2^DΩ ≢ 1 (mod 3)`): branch `s₀ = 1` if `y₀ ≡ 1 (mod 3)` else `2`, depth `v₃(2^{s₀}y₀ + 1) = v₃(2^{s₀+D}Ω − (2^{s₀} − 1))`, `y₀ = 2^DΩ − 1`.

**Corollary.** For any cap `J ≥ 1`, the branching of a node to one level — which doors are alive, the branch `s₀` on each, and every child's depth capped at `J` — is a function of `D` and `Ω mod 3^J`, and only the door `a = 1`, the top door, and (at `D = 1`) the cascade child can have depth above `1`.

**Proof.** The cascade child is Theorem 14.10.1 applied with the node's own exit door and `d = D`: the `d ≥ 2` branch gives depth exactly `1`, the `d = 1` branch gives `1 + v₃(4Ω − 1)`, verbatim. For the door children: `y_a + 1 = 2^{D−a}3^aΩ`, so for `a ≥ 1` this is `≡ 0 (mod 3)`, forcing `y_a ≡ 2 (mod 3)` and the lowest admissible branch `s₀ = 2` always (Lemma 18.1.2(iii)). The door child's numerator is `4y_a + 1 = 4(2^{D−a}3^aΩ − 1) + 1 = 2^{D−a+2}3^aΩ − 3 = 3·(2^{D−a+2}3^{a−1}Ω − 1)` for `a ≥ 1`, so its depth is `1 + v₃(bracket)`, bracket `= 2^{D−a+2}3^{a−1}Ω − 1`. For `a ≥ 2` the bracket has `3 | 2^{D−a+2}3^{a−1}Ω` (since `a − 1 ≥ 1`), so bracket `≡ −1 (mod 3)` and `v₃ = 0`: depth exactly `1`. For `a = 1` the bracket is `2^{D+1}Ω − 1` (no forced factor of `3`), giving depth `1 + v₃(2^{D+1}Ω − 1)`. For `a = 0` (if alive): `y₀ = 2^DΩ − 1` has no forced residue mod `3`, so `s₀ ∈ {1,2}` genuinely depends on `y₀ mod 3`; the numerator is `2^{s₀}y₀ + 1 = 2^{s₀+D}Ω − (2^{s₀} − 1)`, whose `v₃` is the depth. For the corollary: liveness of every door and `s₀` of the top door are functions of `Ω mod 3` (the `a ≥ 1` doors) or `Ω mod 3` again (the top door's own residue); every `a ≥ 2` depth is `1` regardless of `Ω`; the `a = 1`, top-door and (`D = 1`) cascade depths, capped at `J`, are functions of `Ω mod 3^J`, since `2^{D+1}`, `2^{s₀+D}` and `4` are units mod every power of `3`, so agreement on `Ω mod 3^J` pins the relevant quantity mod `3^J`, which pins its `v₃` whenever that is `< J` and pins "`≥ J`" otherwise. ∎

**Reconciliation with the census (18.5).** Under the depth ledger `P(D=j) = 2·3^{−j}` the mean number of `a ≥ 2` door children per node is `1/6` (mean `D = 3/2`; mean live doors `= 1`; the ledger's mass on `(D−2)^+` is `1/6`), so the deterministic-depth-`1` doors are a minority of a node's door children — measured on the level-`18` census (`124,068` nodes): `20,431` of `123,439` door children have `a ≥ 2`, a share `0.1655` against the ledger's `1/6 = 0.1667`.

**Verified** — `experiments/comb_signatures.py`, canaries and section (a): the children of `(7,3)`, `(1,2)`, `(5,2)` against the four laws; the four laws directly, on `5,000` random states (`Ω < 10^6`, `D ≤ 30`), `0` failures; the corollary on `2,000` pairs at `J ∈ {2,3,4}`, sufficiency `2,000/2,000` at every `J` (the "one digit fewer" lift differs at a declining rate as `J` grows: `642/2000`, `241/2000`, `73/2000`); every `a ≥ 2` door found alive with depth exactly `1` on the whole level-`18` census (`124,068` nodes); the `a ≥ 2` share `0.1655` against `1/6`.

### 18.9.2. The signature and its digit budget

**Definition 18.9.2.1 (the signature).** For a comb node `v`, integers `k ≥ 0` and cap `J ≥ 1`, the **signature to depth `k` with cap `J`**, written `ζ_k^J(v)`, is defined recursively: `ζ_0^J(v)` is a fixed leaf marker; for `k ≥ 1`, `ζ_k^J(v)` records the type of every edge out of `v` — one cascade edge to the cascade child `c`, labelled by `min(depth(c),J)` and `ζ_{k−1}^J(c)`, and for every door `a = 0,…,D(v)−1`, either `dead` or, if live with child `c`, the label `(a, s₀, min(depth(c),J), ζ_{k−1}^J(c))` (the root's own excluded self-loop, 18.2.1, carries no label). Cores are not part of the signature: two nodes with equal signatures branch identically to depth `k`, every depth read only up to the cap.

**Proposition 18.9.2.2 (sufficient budget).** Define recursively `χ(v,0) = 0` and, for `k ≥ 1`,

```text
χ(v,k) = max( J,  max over live children c of v of  [ χ(c,k−1) + cost(v → c) ] ),
cost(door edge into door a, child depth d′)  = d′ − a,
cost(cascade edge, child depth d′)           = d′ − D(v),
```

where `d′` is the child's **real** depth when `k ≥ 2` (the recursion continues past `c`) and the **capped** label `min(d′,J)` when `k = 1` (the recursion stops at `c`, and 18.9.1's corollary already shows the capped label costs no more). Then `ζ_k^J(v)` is a function of `(D(v), Ω(v) mod 3^{χ(v,k)})`.

**Proof.** Three digit facts, each an exact-division or door-recovery identity already on file: (a) `y_a mod 3^m` is a function of `Ω mod 3^{m−a}` (`y_a + 1 = 2^{D−a}3^aΩ`, `2^{D−a}` a unit mod every power of `3`); (b) the node's own exit `y mod 3^m` is a function of `Ω mod 3^{m−D}` (`y = (3^DΩ − 1)/2^s` exactly, `2^s` a unit); (c) for a child born at branch `s′` on a door `z` (`2^{s′}z + 1 = 3^{d′}Ω(c)` exactly), `Ω(c) mod 3^n` is a function of `z mod 3^{n+d′}` (`2^{s′}` a unit, then exact division by `3^{d′}`). Composing (c) with (a) or (b): `Ω(c) mod 3^n` needs `Ω(v) mod 3^{n + cost(v→c,d′)}`, `d′` the child's real depth — exactly `cost` more digits. Induct on `k`: at `k = 1`, `ζ_1^J(v)` is a function of `Ω mod 3^J` by 18.9.1's corollary, matching `χ(v,1) = J` (every `cost` term at `k=1` uses the capped label, already shown sufficient at `J` digits). For `k ≥ 2`, assuming `ζ_{k−1}^J(c)` is a function of `Ω(c) mod 3^{χ(c,k−1)}` for every child `c`, the composition above with the real `d′` gives `Ω(c) mod 3^{χ(c,k−1)}` as a function of `Ω(v) mod 3^{χ(c,k−1) + cost(v→c,d′)}`; the maximum over children and with `J` is exactly `χ(v,k)`. ∎

**The capped-cost correction.** A first attempt used the capped label `min(d′,J)` uniformly in `cost`, at every `k`, on the reasoning that the signature only ever records depths up to `J`. This under-counts: knowing a child's depth is "`≥ J`" is cheap (`J` digits, 18.9.1's corollary), but *reconstructing `Ω(c)` itself* — needed once `k ≥ 2` — goes through exact division by `3^{d′}` with `d′` the child's **real** depth, not knowable from the capped label alone. The bug was caught by its own sufficiency check: under the capped-cost formula, `1,500` sufficiency pairs at `k = 2,3,4` failed at `4`, `31` and `84` pairs respectively (`0` at `k = 1`, where capping is exact); with the real-depth correction the same test found `0` failures at every `k ≤ 4`. The fix is exactly what the proof requires: cap the cost only at the base case, where the recursion does not continue.

**Measured.** Sufficiency `1,500/1,500` at every `k ∈ {1,2,3,4}` (`J = 3`); mean `χ` grows with `k` (`3.00, 3.75, 5.26, 6.86`; max observed `3, 8, 13, 14`); the "one digit fewer" lift differs from the base signature in a growing but minority fraction of pairs (`185/1500` at `k=1` to `373/1500` at `k=4`) — the budget is sufficient but not sharp per node, as the pre-check anticipated.

### 18.9.3. The exact signature modulus

**Definition 18.9.3.** For a node `v` and `k`, `ψ(v,k)` is the least `M` such that every valid state `(Ω′,D(v))` with `Ω′ ≡ Ω(v) (mod 3^M)` has the same signature `ζ_k^J` as `v`. Sufficiency at `M = χ(v,k)` (Proposition 18.9.2.2) bounds the search, `ψ(v,k) ≤ χ(v,k)`: at each candidate `M ≤ χ(v,k)`, sufficiency is decided exactly by testing every one of the `3^{χ(v,k)−M}` completions of the low-`M`-digit prefix into a full `χ(v,k)`-digit residue. Sufficiency at `M` trivially implies sufficiency at every `M′ ≥ M`, so `ψ(v,k)` is well defined as the least such `M`.

**A caution, checked rather than assumed.** Whether sufficiency at `M` can be certified more cheaply — by testing only the two sibling lifts of `Ω(v)`'s own digit at position `M−1`, holding every other digit at `Ω(v)`'s actual value, rather than all `3^{χ−M}` completions — is not established here: such a test only shows `ψ(v,k) > M−1` when it finds a difference; it does not prove sufficiency for the *whole* coset, since a digit irrelevant against `Ω(v)`'s own higher digits could still matter against a different combination of them. `ψ(v,k)` is therefore computed by the full search, not the cheaper one; it stayed small enough in every sampled node (`χ ≤ 14`) that the search was never capped.

**Measured.** `ψ(v,k)` computed exactly for a level-based sample (`k = 2`, `J = 3`; up to `2,000` nodes per level, all `478` at level `10`) at levels `10, 12, 14, 16, 18` — mostly `D = 1`, the ledger's most populous class: `χ − ψ ∈ {0,1}` in every one of `8,456` sampled nodes (mean `0.631` overall; `χ = ψ` exactly in `37%`). Extended to random states with `D ∈ {2,…,6}` and `k ∈ {2,3}` (`300` states per `(D,k)` pair, `2,700` states total): the slack is `0` or `1` at every one of them too, no exception, including a direct check of seven states independently flagged at review as candidates for larger slack — recomputed here, all seven have slack `0` or `1` under this section's own `χ` (Proposition 18.9.2.2: real child depth once `k ≥ 2`, the capped label only at the base case), and the exact `ψ` found for each matches the value independently reported at review; the larger slack independently reported for the same seven states traces to a different (also valid, but looser) sufficient bound that does not cap at the base case — see the findings for the full account. Over `12,700` states combined (the level sample plus the extension), the slack never exceeds one digit; which structural feature accounts for it (a capped depth needing fewer digits than the real value; a maximum over children overcounting on all but the binding one) is not separately isolated — the measurement settles only the slack's size, not its cause.

### 18.9.4. The search: is any grouping finer than residues?

For levels `λ ∈ {10,12,14,16,18}` of the positive comb (18.5) and `k ∈ {1,2,3,4}` (`J = 3`):

**C.1 Descriptive counts.** Restricted to `D = 1` nodes (the ledger's most populous class) at each level, with `k = 2`: the sample's distinct exact classes (18.9.3) and distinct signatures are counted directly — e.g. level `18`: `1,500` nodes fall into `53` classes carrying `31` distinct signatures, `20` of which are shared by two or more classes, the largest such group `4` classes. **This is a count, not a test.** An earlier version of this item compared the observed same-signature-different-class count against a "birthday-style independence baseline" `C(C−1)/2·Σ_s q_s²` built from the same classes' own signature frequencies; that baseline is algebraically close to the observed count itself and tests nothing — writing `f_s` for the number of classes carrying signature `s`, the observed count is `Σ_s C(f_s,2) = (Σf_s² − C)/2` exactly, and the plug-in baseline evaluates to `≈ (Σf_s²)/2` for the same `f_s`, the two differing by a fixed `≈ C/2` that algebra alone produces, not anything the data could have shown otherwise — and is withdrawn. No permutation test rescues this either: permuting which class carries which signature label permutes one fixed multiset of labels among positions, and `Σ_s C(f_s,2)` is a function of that multiset alone, identical for every permutation of it — there is no null distribution here to compare against. Testing whether any grouping finer than the exact classes predicts branching needs a candidate predictor to test the signature *against*; C.2 supplies one.

**C.2 The 2-adic probe.** Nodes are grouped by `(D, ζ_1^J)`; within each group of `≥ 20` members and at each `m` from `2` to `10`, `Ω mod 2^m` is tested against the `D`-matched population marginal (the fair baseline — `D` is already part of the grouping) via a standardized residual per cell, **admitting only cells with expected count `≥ 5`**: an earlier version tested every cell to `m = 8` regardless of group size, including groups of `~20` nodes over `2^7` odd residues — expected count `~0.1` per cell, where the residual statistic is meaningless (one observed node already gives a residual near `3`, two near `7`, with no structure required); its two apparent threshold exceedances were this design artefact and are withdrawn. With the size floor, the achieved range is `m ∈ [2,10]` at every level (small groups support only small `m`; the large `D ≤ 2` groups support the whole range), `121`–`8,604` valid cells per level, and the largest standardized residual at any level is `3.13`, against Bonferroni-corrected thresholds of `4.5`–`5.4` — no exceedance at any level. **A clean negative over the range actually tested, `m ∈ [2,10]`: the 2-adic residue carries no detectable information about the 3-adic branching signature, at the sampling noise floor** — the same reading as ladder.md 15.6.7's tear-line measurement.

**C.3 Signature ledgers.** (a) Distinct signatures per level against distinct `(D, Ω mod 3^χ)` classes occupied (the sufficient `χ`, an upper bound per 18.9.3's slack on the true class count): `19` signatures / `66` classes at level `10`, growing to `40` / `157` at level `18` — always more classes than signatures, the gap being exactly the coincidences C.1 measures. (b) The depth-1 signature frequency against a product-law prediction (each valuation an independent draw from the canonical law `P(v_3 = t) = 2·3^{−(t+1)}`, the top door alive with probability `1/2` and, if alive, `s₀ ∈ {1,2}` with probability `1/2` each — the census's own unweighted split, 18.5, not the ledger's forward-visit-weighted `2:1`), tested **conditional on `D`**: an earlier version integrated `D` over the depth ledger for the census and compared against a `D`-uniform box, which compares depth mixes rather than shapes whenever the two populations' `D`-mixes differ, as here — withdrawn. Fixed at `D = 1` and `D = 2` separately, with the box re-drawn at that fixed `D` (not uniform `D`): the box and the census agree closely with each other at every one of the four dominant shapes at each `D` (e.g. `D=1`'s top shape: census/predicted `1.499`, box/predicted `1.503`) — so the earlier apparent census-vs-box gap was entirely the `D`-mix confound, and 18.5's cascade-overrepresentation reading is not a live explanation once `D` is fixed on both sides. The **product law itself does not hold** at either `D`: ratios from `1.5` to `18.3` across the four dominant shapes, box and census alike — the joint valuations entering one signature (the cascade or `a=1` door's depth, the top door's liveness and its own depth) are not independent even under this "generic" model, a fact about the model's assumption, not a new fact about the comb. (c) The window ledger, on `8` random doors per `k ∈ {2,3}` (window widths `9, 27`): the **depth-label multiset** of the window's nodes is exactly door-independent — identical across all `8` doors at both `k`, matching 14.6.5.2's own exact count. The **full depth-1 signature multiset** is *not*: `7` of `8` windows (`k=2`) and `5` of `8` (`k=3`) gave distinct multisets. **A precise answer: the window ledger is exact one level down (the depth, 14.6.5.2) and only statistical past it (the full signature) — no new exact law at this seam.**

**Reading, stated once.** Ladder.md 15.6.7 measured the successor core's residue after a step at the sampling noise floor; the expectation for this item was the same. Of the three sub-items, C.2 is the one that tests a candidate predictor (`Ω mod 2^m`) against the signature directly, and it bears the expectation out over the range it validly covers (`m ∈ [2,10]`): no detectable dependency. C.1 is a descriptive count, not a test, and supports no claim either way. C.3(c) answers a narrower, exact question directly: the window ledger holds for the depth label (14.6.5.2) and not for the full signature. C.3(b) finds that a simple product-law model of shape frequency fails, conditional on `D`, on both the census and the box alike — a fact about that model, not a new grouping of states. Taken together, over the ranges and depths actually tested: no grouping finer than the residue classes `(D, Ω mod 3^{χ(v,k)})`, or their exact refinement `(D, Ω mod 3^{ψ(v,k)})`, was found to predict branching. The section closes "patterns in the branching beyond residues" on the comb, to the tested range, as a negative result where a test was actually run (C.2, C.3(c)), and as an open descriptive question where one was not (C.1).

**Verified** — `experiments/comb_signatures.py`, section (c): C.1's exact classes and signature counts at every level (descriptive); C.2's contingency residuals at every level over `m ∈ [2,10]`, cells with expected count `≥ 5` only, with the Bonferroni comparison; C.3(a)'s counts, C.3(b)'s `D`-conditional predicted-vs-census-vs-box table at `D = 1, 2`, C.3(c)'s window multisets on `8` doors per `k`; the whole-level-`18`-census sanity that every `a ≥ 2` door has depth exactly `1` (`124,068/124,068`); the extended slack measurement of 18.9.3 (`2,700` states, `D ∈ {2,…,6}`, `k ∈ {2,3}`, plus the seven states checked explicitly at review).

### 18.9.5. Standing

What is now known about grouping states by branching: exactly the residue classes `(D, Ω mod 3^{χ(v,k)})` sufficient by construction (18.9.2), refined at the cost of an exact but expensive search to `(D, Ω mod 3^{ψ(v,k)})` (18.9.3), and nothing finer over the range 18.9.4 actually tested for it (C.2, `m ∈ [2,10]`; C.3(c), the window ledger) — C.1 is descriptive and settles nothing either way, and C.3(b) characterises a model failure, not a new grouping. The level-1 branching laws (18.9.1) are exact and proved; the budget composition (18.9.2) is exact and proved, with one corrected cost formula (the capped-cost bug, caught by its own sufficiency check); the exact modulus (18.9.3) is a measurement, not a theorem, and its slack (`χ − ψ ∈ {0,1}`, confirmed over an extended sample spanning `D ∈ {1,…,6}` and `k ∈ {2,3}`) is reported, not explained. Grade: **formulation plus measurements**; nothing proposed; no cycle search; no front moves; the family frame is not used — grouping is by residue class (18.9.1–18.9.3) and by signature (18.9.4), kept apart throughout.
