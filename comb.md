...
**Why the cost uses the real depth.** The signature records a child's depth only up to `J`, and knowing that a depth is `≥ J` costs `J` digits (18.9.1's corollary); but reconstructing `Ω(c)` itself — needed whenever the recursion continues, `k ≥ 2` — goes through the exact division by `3^{d′}` with `d′` the child's **real** depth, which the capped label does not supply. So the cost is capped only at the base case, where the recursion stops. A budget that caps the cost at every level is not sufficient: on `1,500` pairs at `k = 2, 3, 4` it fails at `4`, `31` and `84` pairs respectively, and at `k = 1`, where capping is exact, at none.
...
**Measured.** `ψ(v,k)` computed exactly for a level-based sample (`k = 2`, `J = 3`; up to `2,000` nodes per level, all `478` at level `10`) at levels `10, 12, 14, 16, 18` — mostly `D = 1`, the ledger's most populous class: `χ − ψ ∈ {0,1}` in every one of `8,456` sampled nodes (mean `0.631` overall; `χ = ψ` exactly in `37%`). On random states with `D ∈ {2,…,6}` and `k ∈ {2,3}` (`300` states per `(D,k)` pair, `2,700` states) the slack is again `0` or `1` at every state. Over `12,700` states combined the slack never exceeds one digit; which structural feature accounts for it (a capped depth needing fewer digits than the real value; a maximum over children overcounting on all but the binding one) is not separately isolated — the measurement settles only the slack's size, not its cause.
