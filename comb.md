...
**Measured.** `ψ(v,k)` computed exactly for a level-based sample (`k = 2`, `J = 3`; up to `2,000` nodes per level, all `478` at level `10`) at levels `10, 12, 14, 16, 18`: `χ − ψ ∈ {0,1}` in every one of `8,456` sampled nodes (mean `0.631` overall, `0.62`–`0.68` per level; `χ = ψ` exactly in `37%` of sampled nodes). The slack, at this `k`, is one digit or none, never more; which structural feature accounts for it (a capped depth needing fewer digits than the real value; a maximum over children overcounting on all but the binding one) is not separately isolated — the measurement settles only the slack's size.
...
**C.1 Coincidences.** Restricted to `D = 1` nodes (the ledger's most populous class) at each level, with `k = 2`: nodes with equal signature but differing at the exact modulus (18.9.3) of either — "coincidences" — are counted and compared with an independence baseline computed from the data (a birthday-style expectation `C(C−1)/2 · Σ_s q_s²` over the `C` distinct exact classes present in the sample, `q_s` the observed fraction of classes carrying signature `s`). At every level the observed count is at or below the baseline (e.g. level `18`: `23` observed classes-pairs against `48.06` predicted; every level near half the baseline). **A clean negative, stated flatly: no grouping finer than the exact residue classes of 18.9.3 is visible in the coincidence rate, at any tested level.**

**C.2 The 2-adic probe.** Nodes are grouped by `(D, ζ_1^J)`; within each group of `≥ 20` members, `Ω mod 2^m` (`m ∈ {4,6,8}`) is tested against the `D`-matched population marginal (the fair baseline — `D` is already part of the grouping, so a `D`-mixed marginal would only detect that `D` correlates with `Ω mod 2^m`) via a standardized residual per cell. Over `15` (level, `m`) combinations, `2` exceed a Bonferroni-corrected threshold at `α = 0.01` (`5.83` against `4.96` at `λ=14,m=8`; `5.18` against `5.11` at `λ=18,m=8`) — consistent with the false-positive rate expected from testing thousands of cells, not a systematic or growing effect (it does not recur at `m=8` for every level, nor at `m=4,6` for the flagged levels). **A clean negative: the 2-adic residue carries no detectable information about the 3-adic branching signature, at the sampling noise floor** — the same reading as ladder.md 15.6.7's tear-line measurement.

**C.3 Signature ledgers.** (a) Distinct signatures per level against distinct `(D, Ω mod 3^χ)` classes occupied (the sufficient `χ`, an upper bound per 18.9.3's slack on the true class count): `19` signatures / `66` classes at level `10`, growing to `40` / `157` at level `18` — always more classes than signatures, the gap being exactly the coincidences C.1 measures. (b) The depth-1 signature frequency against a product-law prediction (each valuation an independent draw from the canonical law `P(v_3 = t) = 2·3^{−(t+1)}`, `D` from the ledger, the top door alive with probability `1/2` and, if alive, `s₀ ∈ {1,2}` with probability `1/2` each — the census's own unweighted split, 18.5, not the ledger's forward-visit-weighted `2:1`): matches to within an order of magnitude on the uniform box population of 18.5 (ratios `0.07`–`2.8` across the eight most frequent level-`18` shapes) but not exactly, itself unsurprising (several joint valuations of one `Ω` are not fully independent even under a "generic" reading) and outside what this item searches for. The **census** deviates from the product law far more, and systematically in the direction 18.5 already names (favouring shallow, simple shapes): every node has exactly one cascade child, so the census over-represents shapes reachable by short cascade runs, the same deviation 18.5's own run-length table already reports for depths alone. Nothing new. (c) The window ledger, on `8` random doors per `k ∈ {2,3}` (window widths `9, 27`): the **depth-label multiset** of the window's nodes is exactly door-independent — identical across all `8` doors at both `k`, matching 14.6.5.2's own exact count. The **full depth-1 signature multiset** is *not*: `7` of `8` windows (`k=2`) and `5` of `8` (`k=3`) gave distinct multisets. **A precise answer: the window ledger is exact one level down (the depth, 14.6.5.2) and only statistical past it (the full signature) — no new exact law at this seam.**

**Reading, stated once.** Ladder.md 15.6.7 measured the successor core's residue after a step at the sampling noise floor; the expectation for this item was the same, and it is borne out at every one of the three measurements: no grouping finer than the residue classes `(D, Ω mod 3^{χ(v,k)})`, or their exact refinement `(D, Ω mod 3^{ψ(v,k)})`, was found to predict branching, at any tested depth or level. The one place a genuinely *exact* further law might have lived — the window ledger, C.3(c) — was tested directly and found not to hold past the depth already proved exact. The section closes "patterns in the branching beyond residues" on the comb, to the tested depth, as a negative result.

**Verified** — `experiments/comb_signatures.py`, section (c): C.1's exact classes, baselines and counts at every level; C.2's contingency residuals at every `(λ,m)` with the Bonferroni comparison; C.3(a)'s counts, C.3(b)'s predicted-vs-census-vs-box table (top-8 census mass `0.9144`, predicted `0.3615`, box `0.0811`), C.3(c)'s window multisets on `8` doors per `k`; the whole-level-`18`-census sanity that every `a ≥ 2` door has depth exactly `1` (`124,068/124,068`).

### 18.9.5. Standing

What is now known about grouping states by branching: exactly the residue classes `(D, Ω mod 3^{χ(v,k)})` sufficient by construction (18.9.2), refined at the cost of an exact but expensive search to `(D, Ω mod 3^{ψ(v,k)})` (18.9.3), and nothing finer (18.9.4's three negative measurements). The level-1 branching laws (18.9.1) are exact and proved; the budget composition (18.9.2) is exact and proved, with one corrected cost formula (the capped-cost bug, caught by its own sufficiency check); the exact modulus (18.9.3) is a measurement, not a theorem, and its slack (`χ − ψ ∈ {0,1}` at the tested `k`) is reported, not explained. Grade: **formulation plus measurements**; nothing proposed; no cycle search; no front moves; the family frame is not used — grouping is by residue class (18.9.1–18.9.3) and by signature (18.9.4), kept apart throughout.
