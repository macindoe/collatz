...
## 10.10. Status of These Observations
...
This subsection will collect the currently observed tower types.

Provisional archetypes:
- **global anchor model:** the family `ω = 1`,
- **regular congruence-generated towers:** currently `ω = 25, 49, 73`,
- **irregular or mixed towers:** currently `ω = 17, 41`,
- **collapsed families:** cases in which factors of `3` are absorbed before a genuinely new reduced family appears.

Possible table format:

```text
family type                 sample ω        first visible lifting class
global anchor               1               d0 = 0
regular congruence tower    25, 49, 73      one exceptional class mod 16, then clean refinement
irregular tower             17, 41          one visible lifting class, but mixed deeper refinement
collapsed / absorbed        multiples of 3  not a genuinely new reduced family
```

Intended role:

* to centralize the current taxonomy while keeping the local families available as worked examples.
...
This subsection will summarize what the tower map changes about the Route A program.

Provisional contents:

* Stage 1 is no longer just the search for isolated formulas for `s`,
* it is now the search for exact control of tower structure on natural infinite families,
* especially:

  * identifying when the congruence tower exists in a clean compatible form,
  * distinguishing regular from irregular refinement,
  * and converting tower control into exact `3`-gain and `d_+` statements.

Intended role:

* to restate Route A in light of the tower picture now emerging.

##### 11.8.2.7. Status of the synthesis map

This subsection will state explicitly what parts of the tower map are formal and what parts remain empirical.

Provisional distinction:

* **formal:** first-layer mod `8` classification; even-branch congruence reformulation in base `9`;
* **empirical but strongly supported:** regular congruence-generated towers in `ω = 25, 49, 73`; irregular towers in `ω = 17, 41`;
* **conjectural:** the broader classifier governing regular versus irregular behavior.

Intended role:

* to preserve logical clarity while allowing the synthesis map to guide later work.
