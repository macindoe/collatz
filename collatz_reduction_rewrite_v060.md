...
## 10.1. Historical role of the early empirical heuristics
...
## 10.2. Residue organization as a heuristic prompt
...
## 10.3. Parity regularity and `3`-gain
...
## 10.4. Finite-state shadow as a long-range question
...
## 10.5. Status and pointer to Appendix A
...
#### 11.8.1.2. What remains load-bearing in Stage 1

Once the later lifting-branch synthesis is in view, the formal role of the preceding family examples is limited. They motivate, but no longer carry, the main argument.

What the main text still needs from Stage 1 is:

- a coarse first-layer partition of the valuation problem,
- a precise identification of the valid lifting branch,
- and an exact arithmetic mechanism generating deeper lifting there.

The next subsections isolate those ingredients.
...
By Proposition `11.8.1.4.1`, the genuinely lifting branch in the valid reduced setting is

```text
ω ≡ 1 (mod 8),   d even.
```

This is the branch on which the first-layer congruence forces
...
The key point is that on the even branch the valuation problem becomes a congruence problem in the exponent variable.
...
**Definition 11.8.1.8.4 (regular and irregular tower types).** A distinguished congruence tower is called regular if, empirically, at each stage one branch stabilizes at a fixed valuation while a unique deeper branch continues to lift. In the current data, this is the behavior observed for
...
still appear to admit a distinguished congruence branch, but the surrounding visible refinement is not cleanly one-branch-at-a-time. These cases will be referred to provisionally as irregular.
...
#### 11.8.1.9. Summary of the Stage 1 entry map

At the level needed for the main line of Route A, Stage 1 now has the following shape:

* first separate the valuation problem by the coarse mod `8` classification,
* then restrict to the valid lifting branch
  `ω ≡ 1 (mod 8), d = 2n`,
* then rewrite the lifting condition as the congruence
  `9^n ≡ ω^(-1) (mod 2^k)`.

The worked examples that historically revealed these steps are now collected in Appendix A. The next subsection no longer needs to rediscover the tower picture family by family. It can proceed directly to its synthesis.

### 11.8.2. Pre-synthesis tower map

The purpose of this subsection is no longer to carry the main formal burden of Route A. That burden now belongs to `11.8.3`, where the lifting-branch synthesis is stated directly.

Instead, the present subsection records the tower picture at the level needed before that synthesis begins. It serves three narrower purposes:

1. to gather the structural outputs of `11.8.1` into one map,
2. to distinguish the exact arithmetic mechanism from the still-empirical tower geometries,
3. and to isolate the right handoff point into the formal Stage 1 synthesis.

At this stage, the picture has three layers:

1. a first-layer partition of the valuation problem by the residue class of `ω (mod 8)`,
2. a lifting-branch congruence mechanism on the even branch, expressed by
   `9^n ≡ ω^(-1) (mod 2^k)`,
3. and a visible tower geometry that, in the current data, separates into several archetypes.

The point of the present subsection is not to prove the full synthesis again, nor to leave the tower picture as a collection of family examples. It is to state the map cleanly enough that `11.8.3` can begin from the right conceptual footing.

#### 11.8.2.1. First-layer partition of the valuation problem
...
is already determined by the residue class of `ω (mod 8)` together with the parity of `d`.

By Proposition `11.8.1.4.1`, this first layer separates the reduced state space into non-lifting branches and a genuine lifting branch. In the valid reduced setting with `3 ∤ ω`, the lifting branch is
...
The remaining residue classes produce only the lower first-layer outcomes:

```text
s = 1, 2,
```

or the parity-switched analogue of the lifting condition in the branch `ω ≡ 3 (mod 8)`.

So the first-layer classification should be read as a gatekeeper. It does not yet describe the full valuation, but it identifies where deeper lifting can occur and thereby isolates the natural branch on which Stage 1 must concentrate.

#### 11.8.2.2. Lifting branch and congruence-tower mechanism

Once one restricts to the valid lifting branch and writes

```text
d = 2n,
```

the valuation condition becomes an exponent-side congruence problem:
...
This is the first exact arithmetic mechanism behind the tower picture. It shows that deeper lifting is not best viewed as an unexplained pattern in the raw depth variable `d`, but as a congruence condition in the exponent variable `n` for the base `9`.

Accordingly, the raw level-`k` tower data attached to a lifting family `ω ≡ 1 (mod 8)` may be recorded as the solution classes

```text
T_k(ω) = { n (mod 2^(k-3)) : 9^n ≡ ω^(-1) (mod 2^k) }.
```

Equivalently, these induce corresponding depth-side classes

```text
d = 2n.
```

At this level of the discussion, the key point is simply that the visible tower structure already has an exact arithmetic generator. The later synthesis will show that this mechanism carries much more of the family dependence than the earlier family-by-family viewpoint suggested.
...
**(b) Regular congruence-generated towers.**
The families
...
The current evidence suggests that this halo is not arbitrary family-specific noise. After centering at the primary spike, the same dyadic shell template appears repeatedly across the irregular examples.
...
This taxonomy is still partly empirical. Its purpose here is organizational: it tells the reader what the later synthesis must explain.
...
* the regular versus irregular geometric distinction,
* the translated-shell relation among irregular families,
* the centered dyadic halo template seen in the irregular branch,
* and the possibility of a finer arithmetic classifier separating tower types.

In particular, the now-familiar tentative split
...
should still be treated only as a heuristic classifier at this stage.
...
The next subsection no longer treats the primary branch as merely an empirical feature of some families. It identifies it formally, assembles it into the global `2`-adic anchor, proves the universal local valuation law below spike height, localizes all possible off-spike excess to the boundary shell, and rewrites the remaining valuation refinement in anchor-displacement coordinates.

So the role of the present subsection is complete: it has turned the earlier tower discussion into a structured map, separated its exact arithmetic generator from its visible geometric archetypes, and prepared the reader for the formal Stage 1 synthesis on the lifting branch.

### 11.8.3. Stage 1 synthesis on the lifting branch

The pre-synthesis tower map of `11.8.2` isolates the correct entry point for Stage 1: on the valid lifting branch
...
The inverse-limit construction of `11.8.3.6` may now be sharpened. The compatible congruence classes do not merely define a family of finite truncations; they define the natural additive coordinate on the lifting branch.
...
In this language, the ratio law of Theorem `11.8.3.5.1` is simply the statement that relative branch placement is given by subtraction in the additive coordinate `N`.

#### 11.8.3.8. Anchor-displacement reformulation of spike height

The additive-coordinate theorem of `11.8.3.7.1` shows that the family-dependent anchor is not merely an inverse-limit bookkeeping device. It is the natural exponent-side coordinate on the lifting branch
...
So the family dependence on the lifting branch has been absorbed into translation by the anchor coordinate.
...
On the valid lifting branch

```text
ω ≡ 1 (mod 8),   d = 2n,
```

the family-dependent datum is the primary congruence branch, equivalently the global additive anchor

```text
N(ω) in Z_2.
```

The surrounding local valuation geometry is universal once expressed in displacement from that anchor: below spike height one has the centered halo law

```text
v_2(ω·3^(d_* + Δ) - 1) = 2 + v_2(Δ),
```

all possible off-spike excess is localized to the single boundary shell

```text
v_2(Δ) = H - 2,
```

and on that shell the excess valuation is already controlled by normalized anchor error.

So, at the valuation level, the lifting-branch synthesis now has the following form:

```text
family-dependent additive anchor N(ω)
+ universal local valuation law in anchor displacement.
```

Equivalently, the conceptual picture is:
...
Relative branch placement is additive in the coordinate `N`, and the off-spike halo should be read as universal displacement-from-anchor structure rather than as additional family-specific data.

This is the formal endpoint of Stage 1 on the lifting branch. The next subsection will separate what this synthesis has genuinely reduced from what remains open beyond the present theorem package.

### 11.8.4. What Stage 1 has reduced and what remains

The purpose of `11.8.3` was to state the formal Stage 1 synthesis on the lifting branch directly. The purpose of the present subsection is different. It is to say, as clearly as possible, what that synthesis has already reduced, what parts of the visible tower picture remain empirical, and where the true remaining bottleneck now lies.

This distinction matters because the Route A program has shifted. The central open problem is no longer whether the lifting branch possesses a family-dependent primary branch, nor whether the boundary-shell valuation behaves as though it were controlled by anchor displacement. Those points are now part of the formal Stage 1 synthesis. The remaining issue is whether the surviving conjectural picture can be globalized and then converted into genuinely dynamical control of the reduced step.

#### 11.8.4.1. What Stage 1 has formally reduced

The formal results of `11.8.3` show that, on the valid lifting branch
...
Second, that anchor is not merely an inverse-limit bookkeeping device. It is the natural additive coordinate on the lifting branch. So the primary family dependence has already been compressed into a single arithmetic coordinate.

Third, once a branch point is fixed, the surrounding local valuation law is formal and universal below spike height. If

```text id="ibxl3o"
H = v_2(ω·3^(d_*) - 1),
```

then for every even nonzero displacement `Δ` satisfying
...
So off the spike, within the proved local range, the halo law is already family-independent.

Fourth, all possible off-spike excess beyond that universal law has been localized to a single shell. If a distinct displacement produces valuation at least `H`, then necessarily

```text id="d9t2n3"
v_2(Δ) >= H - 2,
```

and if it produces valuation strictly greater than `H`, then necessarily

```text id="45w1fh"
v_2(Δ) = H - 2.
```

So the unresolved off-spike refinement is not spread across all large displacements. It is concentrated on one dyadic boundary shell.

Fifth, even on that shell, the remaining valuation refinement is no longer independent of the anchor. The boundary-shell correction term admits the exact valuation law

```text id="xsw2by"
v_2(B_H(u,t)) = v_2(q+t),
```

where `q` is the normalized odd part of the anchor truncation error. Thus, at the valuation level, the boundary-shell term is not a second family-specific invariant independent of the anchor. Its valuation is already controlled by anchor displacement.

Taken together, these results show that Stage 1 has formally reduced the lifting-branch valuation picture to:
...
That is the main formal output of Stage 1.

#### 11.8.4.2. What remains empirical, but is now sharply organized

Although the formal Stage 1 synthesis is already substantial, not every feature of the visible tower geometry has yet been derived at the same level.

What remains empirical, but is now organized much more sharply than before, includes the following:

* the regular congruence-generated tower geometry seen in the families
  `ω = 25, 49, 73`,
* the irregular halo-tower geometry seen in the families
  `ω = 17, 41, 65, 89`,
* the translated-shell relation among irregular examples,
* the continued centered dyadic halo template beyond the presently proved local-height regime,
* and the visible interpretation that, in the irregular branch, family dependence appears to be exhausted by anchor placement together with a universal off-spike law.

These should no longer be regarded as disconnected observations. The formal results of `11.8.3` now explain a substantial part of what these computations were pointing toward. What remains empirical is not the existence of the anchor mechanism itself, but the extension of the local anchor-displacement picture into a global one.

So the current empirical picture is sharper than earlier drafts allowed. It is no longer “there seem to be mysterious irregular families.” It is:

```text id="jlwm9v"
formal anchor mechanism
+ formal local halo law
+ strong empirical evidence that the same pattern persists globally.
```

#### 11.8.4.3. What remains genuinely open

After the Stage 1 synthesis, the genuinely open questions on the lifting branch are now narrower.

**(a) Globalization of the off-spike halo law.**
The local centered halo law is proved only below spike height. What remains open is whether the same anchor-displacement law continues to govern the full off-spike picture without the present local-height restriction.

Equivalently: does the local theorem already capture the true global off-spike law, or does new structure eventually appear beyond the proved range?

**(b) Intrinsic form of the correction term beyond valuation.**
...
**(c) Global tower-type classification.**
The current data still suggests a distinction between regular and irregular visible tower geometries, and it still suggests a possible arithmetic classifier. But no full theorem yet separates these tower types globally.
...
**(d) Conversion from valuation synthesis to reduced dynamics.**
...
This is why the next stages of Route A remain dynamical rather than purely valuation-theoretic.

#### 11.8.4.4. The true remaining bottleneck after Stage 1

The main effect of Stage 1 is that it changes what should count as the bottleneck.

The bottleneck is no longer:

* to discover whether there is a primary branch,
* to decide whether the anchor is real,
* or to test whether the boundary-shell valuation term behaves independently of that anchor.

Those are no longer the live edge.

The bottleneck is now twofold:

1. **globalize** the anchor-displacement picture beyond the presently proved local regime;
2. **translate** that valuation picture into control of the reduced transition itself.

In other words, the next theorem target is not simply another valuation theorem. It is a passage:
...
That is the correct interpretation of what Stage 1 has reduced. It has not solved the reduced dynamics, but it has moved the remaining burden away from raw family-dependent shell phenomenology and toward globalization plus dynamical conversion.

So the next task is no longer to determine whether a second valuation invariant survives beyond the anchor. The next task is to decide whether the remaining conjectural picture can be globalized and whether the correction term itself admits a cleaner intrinsic description beyond its valuation shadow.

#### 11.8.4.5. Compact status snapshot

At this point, the status of Stage 1 may be summarized as follows.
...
formal:
    ...
    boundary-shell valuation controlled by anchor error

empirical but strongly supported:
    regular congruence-generated towers,
    irregular congruence-centered halo towers,
    centered dyadic halo persistence beyond the proved local range,
    anchor placement as the visible family-dependent datum in the irregular branch

open:
    globalization of the off-spike halo law,
    intrinsic form of the correction term beyond valuation,
    global tower-type classification
```

This compact snapshot should be read only as a checkpoint. Its purpose is not to replace the fuller discussion of `11.8.4.1`–`11.8.4.4`, but to gather that discussion into one visible ledger before Route A passes on to Stage 2.

**Update (v060).** The item "conversion of the Stage 1 valuation synthesis into control of the reduced transition" has been removed from the open list. Section `11.8.5` now provides that conversion: Theorem `11.8.5.2.1` gives the exact `3`-gain law for all lifting-branch families, with the `ω = 1` case and the spike-level criterion as explicit corollaries. The remaining open items above belong to Stage 3.
...
### 11.8.5. Stage 2: Convert Valuation Results into `3`-gain Theorems
...
The Stage 1 synthesis has already assembled the two ingredients required to prove an exact theorem of this kind, and the combination is immediate.
...
**Interpretation.** The `3`-gain question on the lifting branch is completely determined by the parity of `v_2(ε)`, which measures how closely `n` approximates `N(ω)` in `2`-adic terms. Once the anchor `N(ω)` is known, the `3`-gain pattern at every non-anchor depth is explicit.

In particular, `3`-gain and no-`3`-gain depths alternate in a regular dyadic pattern: `v_2(ε) = 1` gives gain, `v_2(ε) = 2` gives no gain, `v_2(ε) = 3` gives gain, and so on. The two sets are each infinite and together partition all non-anchor depths on the lifting branch.
...
This criterion is exact and readable directly from the spike level, without computing `s` at the depth. It applies uniformly to all lifting-branch families, regular and irregular.

#### 11.8.5.5. Scope and the remaining open question

Theorem `11.8.5.2.1` is family-uniform: it holds for all `ω ≡ 1 (mod 8)` on the lifting branch. The distinction between regular and irregular tower types does not affect the `3`-gain criterion itself.

What the theorem does not close is the arithmetic of `N(ω)` for the irregular families. For regular families, `N(ω)` is approached through a clean nested congruence tower, and `v_2(ε)` at each depth is readily computable from the tower level. For irregular families, the halo structure around the spike sequence reflects the same underlying anchor, but the precise `2`-adic expansion of `N(ω)` for those families carries structure that is not yet fully understood.

The `3`-gain law is therefore closed as a formal theorem. The open question is whether the `2`-adic expansion of `N(ω)` for irregular families has further arithmetic organization — a question that belongs to the study of `C` and is the natural entry point for Stage 3.

### 11.8.x Stage 3. Depth Evolution Through `d_+ = v_2(C) - s + v_3(C)`
...
### 11.8.y Stage 4. Only Then Refine `ω_+`
...
The main text now carries a sharper synthesis: on the lifting branch `ω ≡ 1 (mod 8)` with `d = 2n`, the valuation problem is governed by the congruence
