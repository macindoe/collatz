...
##### 11.8.2.6.3. Spike height and the dyadic refinement barrier

The centered halo law yields an exact divisibility barrier for when a distinct refinement of the branch can first occur.

**Corollary 11.8.2.6.3 (spike-height divisibility barrier).**
...
so Theorem `11.8.2.6.2` applies and gives

```text
v_2(ω·3^(d_* + Δ) - 1) = 2 + v_2(Δ) < H,
```

contrary to assumption. Therefore one must have

```text
v_2(Δ) >= H - 2.
```

This is equivalent to the divisibility statement
...
**Interpretation.**
This is the exact structural content presently justified by the local theory: the spike height forces a dyadic barrier. No distinct point can match or exceed the spike before the scale `2^(H-2)` in the depth variable.

The stronger statement that the next refinement jump always occurs exactly at that first allowable scale is not proved here. That sharper statement remains empirical. What is proved is the divisibility barrier itself.
...
The preceding results define `N(ω)` intrinsically, but they do not yet identify it in a more explicit arithmetic form.
...
is the natural additive coordinate on the lifting branch characterized by the congruence tower
...
If so, then the primary branch is not merely encoded by `ω` level-by-level: it is given by a single global arithmetic coordinate on the whole lifting branch.
...
> determine whether the local centered halo law extends globally off the spike, so that the family-dependent data in the irregular lifting families is exhausted entirely by the additive anchor coordinate `N(ω)`.

Equivalently, the remaining question is whether the irregular families contain any genuinely additional family-specific valuation structure beyond anchor placement, or whether their apparent complexity is completely accounted for by

```text
anchor location + universal off-spike law.
```

In this form, the main arithmetic issue is no longer the existence or compatibility of the primary branch. That has already been absorbed into the global coordinate `N(ω)`. The remaining issue is whether the empirical global halo picture can be promoted to theorem.
