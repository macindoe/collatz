...
**The statistics door now has an exact address (July 2026).** The equidistribution hypothesis is formalized and calibrated (aeh.md, §13): in the *bulk* — while orbit values are large — every tested statistic matches the exact product law, and the folklore ledger is now a theorem about that law; the wild deviations live entirely in the *bottom*, the fixed drainage basin of small numbers. One flagged anomaly survives all controls (a ~1.8% digit bias in one class at 5σ, §13.5) and is the page's open, falsifiable target. Conditional theorems record exactly what AEH buys (the ledger, the 1/3 rate, drift — almost-everywhere statements only) and what it cannot (individual staircase tails).
