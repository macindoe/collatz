...
### 10.11. Primary congruence branch and 2-adic anchor in the irregular lifting branch

Sections `10.9` and `10.10` suggest that the irregular families are best understood in two layers:
...
The centered halo template appears to be shared across the irregular families once one recenters at the primary spike. Thus the remaining family-specific data is not the shell pattern itself, but the location of that pattern in the depth variable `d`.
...
This shows that the distinguished spike locations in the irregular branch should not be regarded as ad hoc residues detected separately at each modulus. They arise from the same congruence mechanism already visible in the regular branch.

Accordingly, for a fixed irregular family `ω`, define the **primary congruence branch** to be the compatible sequence of solution classes
...
These classes are the natural candidates for the true primary spike locations at level `k`. In particular, the visible depth offsets in the irregular branch are not arbitrary translations extracted from a fixed modulus. They are finite truncations of a compatible lift branch.

This suggests the sharper structural picture:

> the family-dependent object in the irregular branch is the compatible primary congruence branch `d_k^*(ω)`, while the surrounding halo geometry is universal after centering at that branch.

Equivalently, the irregular families appear to be controlled by a `2`-adic anchor rather than by isolated spike positions. One may therefore view the compatible classes `d_k^*(ω)` as successive truncations of a single `2`-adic quantity
...
On the even branch, this is more naturally expressed in the exponent variable `n = d/2`. Writing

```text
n_∞(ω) = d_∞(ω)/2,
```

the anchor condition becomes

```text
9^(n_∞(ω)) = ω^(-1)
```

in `Z_2`, and the finite branches `d_k^*(ω)` are then the depth-side truncations of this `2`-adic exponent.
...
So the irregular branch is provisionally organized as
...
denotes the height of the level-`k` primary spike, then the numerical data indicates that the spike height is governed by how accurately the truncation `d_k^*(ω)` approximates the full anchor `d_∞(ω)`. Equivalently, the jump size in the primary branch appears to encode the next gain in `2`-adic accuracy.
...
while the corresponding spike heights begin

```text
5, 8, 11, 15, 19, 20, ...
```

and the successive branch jumps are
...
So, at least empirically, the spike height appears to track the `2`-adic size of the next refinement jump in the primary branch.

Thus the irregular lifting families are no longer best described as merely having “shifted halo shells.” A sharper provisional description is:
...
The regular lifting families strongly suggest that the even-branch valuation condition is governed by a congruence tower in the exponent variable. Section `11.8.1.7.1` makes this precise: writing
...
For the regular families, the visible lifting geometry appears to be exhausted by this congruence mechanism alone. The irregular families, however, initially seem to display something extra: a distinguished high spike together with a surrounding dyadic halo pattern.

The evidence of Sections `10.9`–`10.11` now suggests that this irregular behavior should not be interpreted as a separate mechanism competing with the congruence tower. Rather, it should be decomposed into two layers:
...
The data suggest that these classes form the true family-dependent object. In regular families, this object seems to account for the entire visible lifting structure. In irregular families, it appears to specify the location of the primary spike, while the off-spike shell structure is then forced by a universal centered dyadic law.

So the irregular branch is now best viewed not as an exception to the base-`9` lifting mechanism, but as its next refinement:

```text id="7ti0gw"
family-specific congruence anchor
+ universal local valuation geometry around that anchor.
```

To isolate the local mechanism, fix an even depth `d_*` and define
...
Thus `H` is the spike height at `d_*`. For an even displacement `Δ`, write
...
Now suppose `Δ != 0`. Since `3^Δ` is odd,

```text id="h7df9i"
v_2(A_*·3^Δ) = H.
```

On the other hand, because `Δ` is even, Proposition `11.8.1.1.1` gives
...
the first term has strictly smaller `2`-adic valuation than the second and so dominates the sum. Hence one obtains the exact local law
...
**Theorem 11.8.2.6.1 (local halo law near a high spike).**
...
Equivalently: once a spike of height `H` is present, every even displacement whose shell valuation lies strictly below `H` is forced to satisfy the same dyadic valuation law as the family `ω = 1`.

This theorem explains the universal centered halo geometry observed in the irregular branch. It also sharply separates the two layers of the irregular picture:

* the **family-dependent anchor data**, encoded by the primary congruence branch and spike height,
* and the **family-independent halo law**, governed only by `v_2(Δ)`.

Thus the halo geometry is no longer the mystery. The remaining arithmetic problem is the branch itself.

This local theorem also clarifies why spike height should control refinement scale. Let `d_*` be a primary spike of height

```text id="d0gc7h"
H = v_2(ω·3^(d_*) - 1).
```

By Theorem `11.8.2.6.1`, every even displacement `Δ != 0` satisfying

```text id="jfv9cx"
2 + v_2(Δ) < H
```

obeys the universal law

```text id="xy1ntt"
v_2(ω·3^(d_*+Δ) - 1) = 2 + v_2(Δ).
```

So no such displacement can produce a valuation as high as the spike itself. The first scale at which a competing depth can even reach the spike height is when
...
Thus the spike height determines the first `2`-adic scale at which the primary branch can refine. This explains why the visible primary branches in the irregular families evolve by long plateaus separated by large dyadic jumps: the local halo law forbids any smaller displacement from competing with the current spike.

Accordingly, if a primary branch jump occurs from one level to the next, its size is naturally expected to be governed by

```text id="gw129y"
Δd  ~  2^(H-2),
```

up to higher-order refinement effects. This is exactly the pattern suggested by the data of Section `10.11`.

For example, in the case `ω = 17`, the branch values

```text id="w1o35e"
4, 12, 76, 588, 8780, 139852, ...
```

have successive jumps
...
at each visible refinement stage.

This motivates the next theorem target.

**Provisional Theorem Target 11.8.2.6.2 (spike-height / jump-size relation).**
If `d_k^*(ω)` is a primary branch point of exact height

```text id="oo1c4l"
H_k = v_2(ω·3^(d_k^*(ω)) - 1),
```

then the next refinement jump in the primary branch occurs at a displacement divisible by

```text id="ycy3v0"
2^(H_k - 2),
```

and in the visible irregular examples appears to occur at exactly that scale.

The significance of this statement is that spike height would no longer be merely descriptive. It would become the arithmetic quantity predicting the next branch-jump size.

Theorem `11.8.2.6.1` also shows that the family-dependent object in the irregular branch is the congruence branch, not the halo. Once one has centered at a spike, the off-spike valuations are universal. Thus the irregular branch is not family-specific in its shell geometry. The family-specific information lies only in the placement of that shell structure.

Equivalently, the true arithmetic problem in Stage `1B` is not to further classify the halo, but to understand the compatible branch

```text id="9u4mr6"
d_k^*(ω),
```

or on the exponent side, the compatible solutions
...
This motivates the interpretation of the branch as a `2`-adic anchor. One should think of the compatible classes

```text id="5uua52"
d_k^*(ω)
```

as successive truncations of a single `2`-adic quantity
...
Under this interpretation, the visible primary spike at level `k` is simply the current finite truncation of the family’s full `2`-adic anchor.

A further consequence is that family-to-family offsets are themselves governed by the same congruence mechanism. If
...
then subtracting exponents gives

```text id="hi5xbl"
9^(n_k(ω_2) - n_k(ω_1)) ≡ ω_1·ω_2^(-1) (mod 2^k).
```

Thus the relative displacement between two primary branches is not an independent phenomenon. It is itself governed by the same base-`9` congruence law, now applied to the ratio

```text id="anaj51"
ω_1·ω_2^(-1).
```

So the offset branch

```text id="eu8l7v"
n_k(ω_2) - n_k(ω_1)
```

should be viewed as the congruence branch attached to that ratio.

This motivates the next exact target.

**Provisional Theorem Target 11.8.2.6.3 (ratio law for relative offsets).**
For any two lifting families `ω_1` and `ω_2`, the level-`k` relative offset in the exponent variable,
...
with the corresponding depth offset given by

```text id="l2t5zt"
d_k^*(ω_2) - d_k^*(ω_1) = 2m.
```

Thus family-to-family displacement is generated by the same congruence tower mechanism as the primary branches themselves.

The structural picture of Route A Stage `1B` is therefore now much sharper than before. The irregular lifting branch is no longer best regarded as an unresolved exceptional zone. Its local geometry has essentially split into:

1. a **proved universal local law** near any sufficiently high spike,
2. a still-open **branch arithmetic problem** for the compatible primary congruence branch,
3. a still-open **spike-height / jump-size law** governing refinement scale,
4. and a still-open but sharply formulated **ratio law** governing relative offsets between families.

So the correct remaining objective is now clear:

> prove that in the irregular branch, the family-dependent data is exhausted by the primary congruence branch, and then identify how that branch is encoded by `ω`.

In this form, the centered halo geometry is no longer the mystery. The remaining mystery is the arithmetic law of the family-specific anchor itself.
