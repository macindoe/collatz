...
Three indexings of the door letter are in play across aeh.md `13.2` and `13.6.3`(i), and no page fixes one. Write `y_i = G^i(x)` for the doors of an odd start `x`, so `y_0 = x` and `y_(i+1) = x_exit(i)`. **(A)** letter `i = stratum(y_i)` — the letter word of `x` itself, and the word itinerary.md Theorem `14.15.1.5` is stated about. **(B)** letter `i = stratum(y_(i+1)) = stratum(x_exit(i)) = (m_(+,i), s_(i+1))` — what `13.2.1`, `13.2.4` and `13.6.3`(i)(a) define, and what reverse.md `14.14.6`'s seam identification supplies. **(C)** letter `i = (m_i, s_(i+1))`, pairing block `i`'s own `m` with block `i+1`'s `s` — not a door stratum, defined on no page, but the pairing `13.2.3`'s clause "`r_i = s_(i+1)`" reads as on its own. The budget sum `Σ_(i<n)(m_i + s_i)` of `13.2.1` and `13.2.3` runs over (A)'s word — `(m_i, s_i) = stratum(y_i)` exactly, which is what makes `x_exit(n−1) = T_1^(S_n)(x)` hold — so its `m_i` is `13.2.4`'s `m_(i−1)`, and the one symbol carries two meanings inside `13.2.3` alone. The measurements are at `briefs/v3r4-fix-findings.md` §3.3 and §8.

The choice is not cosmetic. Under (A) the budget count and the letter word's total exponent are the *same* number, `13.2.3`'s gap is `0`, and `13.2.4`(a) — `TV(Law(ℓ_0, …, ℓ_(n−1)), B^(⊗n)) ≤ 2^(J+2)/N + P_B(S_n ≥ J)` — is exactly `14.15.1.5`. Under (B), which is what `13.2.4` in fact defines, the word (a) speaks of is the letter word of `x` with its *first* letter deleted, whose cylinder classes need that letter's own exponent too; (a)'s tail term is then one letter short, and the printed bound does not hold. At `N = 2^b` and `J = b`, where (a)'s own first term is `0`, the exact total variation of the one-letter law over all odd starts of `[2^b, 2^(b+1))` is `2279/2^20` at `b = 16` and `685/2^21` at `b = 20`, against printed bounds `1/2^11` and `5/2^17` — factors `4.45` and `8.56` over; the same two quantities computed on (A)'s word are `743/2^21` and `227/2^23`, inside the bound in both. No *conclusion* of `13.2.4` is at stake: (b)'s identity is general in `n`, `(g)`'s rate `I(θ, τ)` has the same value at `n = T_N + 1` as at `T_N`, and (c)–(f), Corollary `13.2.4.1` and Theorem `13.6.4` read identically either way; `13.2.4`(g)'s offset clause was written to hold under all three readings and does.

**Open question: which indexing should the record carry, and what has to move with it?** Standardizing on **(A)** makes (a) exactly `14.15.1.5`, makes (c)'s `log₂(y_n + 1) > log₂ N − S_n` exact from `y_0 = x`, and collapses both `13.2.3`'s gap clause and `13.2.4`(g)'s offset parenthetical to nothing; its cost is that `13.2.1`'s letter, `13.6.3`(i)(a)'s letter-per-block identification and Corollary `13.2.4.1`'s "`13.2.1`'s tallied word is the letter word of `13.2.4`(d)" are each stated in (B) and would each have to be restated, together with the seam sentence at reverse.md `14.14.6` they rest on. Keeping **(B)** costs one index in `13.2.4`(a) — `P_B(S_n ≥ J)` becomes `P_B(S_(n+1) ≥ J)`, with a clause in its proof naming the start's own letter — and leaves every other statement where it stands. **(C)** is not a candidate: it is not a door stratum and no page defines it. **Closure is checkable:** the chosen convention written into `13.2.1`, `13.2.3`, `13.2.4` and `13.6.3`(i) with one symbol per quantity, and every statement citing them re-read against it — in particular `13.2.4`(a)'s tail term, `13.2.3`'s gap clause, and `13.2.4`(g)'s offset clause, which no longer needs to hedge across readings once one is fixed.
