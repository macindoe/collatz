...
scope: monolith sections 11-11.7 (section 10 was absorbed into index.md); 11.8 added post-monolith (citation-debt record); 11.10 added post-monolith (per-letter window height laws, itinerary.md 14.15.9; recorded as 11.9 in pre-2026-07-23 briefs); 11.11 added post-monolith (what AEH's budget clause supplies, aeh.md 13.2.3); 11.12 added post-monolith (which door-letter indexing the record standardizes on, aeh.md 13.2 and 13.6.3(i)); 11.13 added post-monolith (must a cycle be top-door, reverse.md 14.8.4 and cycles.md 12.6.1.8); 11.14 added post-monolith (does every family reach the trivial family, stage2.md 11.8.5.6 and ladder.md 15.1.1)
updated: 2026-09-13
...
**Calibration (2026-09-04, shared ledger L-A10).** No altitude of the form `x·f(x mod 2^k)` — `k` fixed, `f` any positive function on `ℤ/2^k` — descends at every step of the Terras half map on the positive integers, for any odd multiplier `p ≥ 3`: Eric Merle's shared-ledger entry L-A10 (correspondence 2026-09-03; his artifact `experiments/run_050.py` at `ericmerle3789/one-obstruction-three-faces-lean` `db0e89d`; `github.com/macindoe/one-obstruction-three-faces`, `LEDGER.md`), two keys (his; ours the approving review on PR #4, posted 2026-09-12) — verified here with fresh code (`experiments/merle_r14_check.py`, 46 checks, 0 failures, 2026-09-04, re-run clean 2026-09-13); it excludes no cycle and closes nothing on this page beyond naming one more finite-state shadow that cannot carry a descent certificate.
...
## 11.13. Must a cycle be top-door?

reverse.md `14.8.4` names the **top-door lineage**: a backward path through top doors only, equivalently (forward) an orbit with no `3`-gain at any step, equivalently (cycles.md `12.6.1.8`) a period-`p` cycle profile `(m_t,s_t)_{t<p}` with every `s_t` odd (the **odd stratum**). The only three known `F`-cycles — `(1,1)`, `(2,1)`, `((4,1),(3,3))` — are all odd-stratum (cycles.md `12.6.1.8`); no cycle with an even `s_t` is known.

**Open question.** Must every period-`p` cycle of the reduced map `F` be top-door — equivalently, must every `s_t` in a cycle's profile be odd? Closure is checkable in either direction: **either** a profile `(m_t,s_t)_{t<p}` with at least one even `s_t` and `q | R_0` (cycles.md `12.6.1`, one condition by `12.6.1.1`'s rotation invariance) — witnessing a cycle outside the odd stratum — **or** a reason ruling every such profile out.

**What is known.** cycles.md `12.6.1.8` and reverse.md `14.8.4`: the odd-stratum simplification of the cycle equation (`ω_r · q = R_r`, no `3^a` factor — a bookkeeping consequence of `s_t` odd `⟺ a_t = 0`, not a new divisibility fact); the bounded canary census (`p ≤ 4`, entries `≤ 6`, `q | R_0`, reproduced and not extended) returns exactly the three known words and their rotations/repeats, no evidence beyond; the odd-step stratum's counting margin (cycles.md `12.6.1.5`) is `0.267` per step against the general `0.079` — the capacity–demand heuristic is, if anything, *less* pessimistic about the odd stratum than about the general family, so the stratum is not disfavored by that heuristic, only unexamined beyond the known instances.

**Calibration.** No mechanism is in hand. The `3^(Σa)` bookkeeping of the general cycle equation (`12.6.1`) is automatically consistent for any realized segment, on or off the stratum — it is not an obstruction a mixed-stratum profile would need to overcome — so a proof in either direction would have to come from the closure condition `q | R_0` itself, not from the per-step laws, which already hold uniformly regardless of stratum. A positive answer (every cycle is top-door) would be a divisibility-aware statement of the kind cycles.md `12.8.5`'s reopen clause names — narrowing the parked condition `q | R_0` to the odd stratum, were the cycle front ever reopened by such an idea. This entry states the question; it answers no part of it and reopens nothing.

**Stopping-rule compliance.** Under the mod-`q` brief's rule (`briefs/modq-spectrum-brief.md`): identities on profiles, no cycle search. The census above is a canary, reproduced at its stated bounds only; no per-period search runs to answer this question, and none is proposed here. Per README's stopping rules and cycles.md `12.8.5`, this entry does not reopen the cycle front.

## 11.14. Does every family reach the trivial family?

A **family** is a core `ω` together with all its depths, the states `(ω,d)`, `d ≥ 1`; its integer members are `2^m 3^a ω − 1`, `m ≥ 1`, `a ≥ 0` — the doors of `(ω, m+a)` (reverse.md `14.1.1`). Write `e(ω,d)` for the block exit `x_exit(ω,d)`.

**The statement (W).** For every core `ω`, some depth `d` has `(ω,d)` reaching `(1,1)` under `F`. The trivial cycle is the depth-`1` member of the trivial family (`ω=1`), which is why the author has always thought in this form. A stronger conjecture — every depth reaches `(1,1)` — implies `W`; `W` does not imply it.

**Three equivalent readings.** **(a)** the exit sequence `e(ω,d)`, `d ≥ 1`, contains a member of the tree from `1` (reverse.md `§14.4`–`14.6`: the positive odd integers whose `T`-orbit reaches `1`). **(b)** the backward tree from `(1,1)` (states whose `F`-orbit reaches `(1,1)`) visits every core at some depth. **(c)** over all odd `x` in the tree from `1`, the numbers `x+1` stripped of their `2`s and `3`s cover every integer coprime to `6`. The three are equal by one short chain: `(ω,d)` reaches `(1,1)` iff `e(ω,d)` is in the tree from `1` (Theorem `9.8.3`, applied to the exit as a representative, since `R = state`); the backward tree from `(1,1)` is by definition `{(ω,d) : F-orbit reaches (1,1)}`, so (b) restates `W` directly; and the `a=0` representative `x=2^dω-1` has `state(x)=(ω,d)`, giving a bijective correspondence between "`(ω,d)` reaches `(1,1)` for some `d`" and "some `x` in the tree has stripped core `ω`" — reading (c).

**Why the record's tools do not reach it.** The family has density zero, so the density bound of `14.6` (and the literature's `x^{0.84}`) says nothing; the digit budget (stage4.md `11.8.7.7`) says no window decides membership; the stronger conjecture implies `W` trivially.

**What the ladder does to it.** By ladder.md `15.1.1`, adjacent exits off-spike are one orbit, so `W` is about the exits at spike depths, one orbit per kick. Two regimes:

* *Cores `ω ≡ 5, 7 (mod 8)`* have no anchor and every spike has `s = 2` — the exact, unconditional first-layer classification (stage1.md `11.8.1.3.1`), not a sampled absence of `s > 2`. Their spike-depth exits obey `e(ω,d+2) = T(6·e(ω,d)+1)` — the kick `6e+1` (`15.1.1`, `s=2`) then one Collatz step, at every spike depth `d`. `W` on these classes says some member of that explicit sequence lies in the tree from `1`.
* *Cores in the lifting classes `ω ≡ 1, 3 (mod 8)`* have kick heights `s` read off the anchor digits (stage1.md `11.8.1.5`). For a kick of height `s ≥ 3`, the kicked exit is `3·2^{s−1}e+1 = 4X+1`, `X = 3·2^{s−3}e`, and the identity `Θ(4X+1) = Θ(X)` (`Θ(y) := (3y+1)` stripped of `2`s, extending `T`'s formula off the odd integers; `3(4X+1)+1=4(3X+1)` makes this exact for every `X`) reduces the kicked exit's orbit, by one step, to the orbit of `X`. **Only at kick height `s = 3`** is `X = 3e` itself odd, so `Θ(X) = T(3e)` and the reduction is to the orbit of `3e` exactly, as the author's original framing has it. For `s ≥ 4`, `X` is even and `Θ(X) = 9·2^{s-3}e+1` in closed form — a specific value generally different from any early term of `3e`'s own orbit (an explicit instance, `e=1,s=4`: `Θ(6)=19 ≠ T(3)=5`, merging only seven steps later). So the clean reduction to "does the orbit of `e` merge with the orbit of `3e`" holds exactly at kick height `3`; at greater heights the one-step-reduced question is of the same shape (an explicit value against a dense set) but not literally the `e`-vs-`3e` pair.

**The wall, stated.** "Do the orbits of `x` and `3x` merge?" — the question kick height `3` reduces to — would follow from the stronger conjecture and is open on its own. Measured (fresh sample, `1,300` random odd `x < 2·10^6` coprime to `3`, `400`-step horizon): the orbits of `x` and `3x` meet above `1` in `1,212` cases and only at `1` in `88` (`1212/1300 ≈ 0.932`) — generic, with no local law. `W` is weaker than the stronger conjecture but of the same kind: an explicit thin sequence must meet a dense set, and no technique exists for that shape. Recorded as the calibration; nothing proposed.

**Flat note.** The trivial family is trivial only at depth `1`: its deeper exits `e(1,d)` are the odd parts of `3^d − 1` (`1, 1, 13, 5, 121, 91, 1093, …`), and whether every one of them reaches `1` is open — the stronger conjecture restricted to that thin sequence.

**Calibration.** No front moves; the fiber-versus-orbit bridge of stage2.md `11.8.5.6` is this question's name in the record, and `11.14` is that bridge's weakest global form.

**Stopping-rule compliance.** Identities and a fresh orbit-merge measurement; no cycle search of any kind, no per-period anything, no equidistribution proof effort, no front reopened.
