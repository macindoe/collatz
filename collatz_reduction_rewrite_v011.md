...
So `u_+` is exactly the odd part of `B`.
...
A = 3^d ω - 1,
s = v_2(3^d ω - 1),
B = 3^d ω - 1 + 2^(v_2(3^d ω - 1)),
```

```text
ω_+ = B / (2^(v_2(B)) 3^(v_3(B))),
d_+ = v_2(B) - v_2(3^d ω - 1) + v_3(B).
```

Equivalently,

```text
F(ω,d) = ( B / (2^(v_2(B)) 3^(v_3(B))),
           v_2(B) - v_2(3^d ω - 1) + v_3(B) )
```
...
So the next reduced state is governed entirely by the valuation profile of the single derived quantity `B`, together with the initial 2-adic valuation `v_2(3^d ω - 1)`.
...
A different kind of exact family appears when one fixes a downstream odd target `x` and studies admissible odd-step predecessors of the form
...
This construction is upstream rather than representative: the values `x_L` need not lie in the same reduced-state family, and in general they do not. Instead, they form a family of structural predecessors of the fixed target `x`, indexed by the odd-step cascade length `L`.
...
Now assume `x` is odd with `3` not dividing `x`, and let `L` be admissible so that `x_L` is integral. Then
...
This gives the `2`-adic behavior of the upstream family.

**Lemma 9.5.2.** If `x` is odd, `3` does not divide `x`, and `L >= 3` is admissible, then

```text
v_2(x_L + 1) = 1.
```

**Proof.** Since `x` is odd and `L >= 3`, the factor `2^(L-1)` is divisible by `4`. Hence
...
so `x 2^(L-1) + 1` is odd. Division by `3` does not affect `2`-adic valuation, so from

```text
x_L + 1 = 2 (x 2^(L-1) + 1) / 3
```

it follows that exactly one factor of `2` divides `x_L + 1`. Therefore

```text
v_2(x_L + 1) = 1.
```

Thus for every admissible `L >= 3`, the upstream family automatically enters the stabilized regime `m_L = 1`.
...
Substituting the definition of `x_L` gives
...
**Proposition 9.5.3.** Let `x` be odd with `3` not dividing `x`, and let

```text
x_L = (x 2^L - 1) / 3
```

be an admissible upstream predecessor. Then for every admissible `L >= 3`,
...
In particular, since `x` is odd,

```text
s_L = L - 1.
```

Thus any admissible odd target `x` gives an exact linear law for `s` along its stabilized upstream predecessor family.
...
varies strictly with `L`. Thus distinct admissible values of `L >= 3` must correspond to distinct reduced-state families.

The running example `x = 23` illustrates this exactly. Since `23 ≡ 2 mod 3`, only odd `L` are admissible. For every admissible `L >= 3`, one obtains
...
The case `L = 1` is exceptional because the stabilized condition has not yet taken over: here `x_1 = 15` and `v_2(x_1 + 1) = 4`.
