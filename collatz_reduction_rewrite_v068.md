...
(Forward note: the synthesis ultimately dissolves this provisional dichotomy. The visible refinement behavior is determined by the digit expansion of the anchor `N(ω)`, and the regular/irregular impression corresponds to high versus low digit density — a continuous statistic, not two classes; see `11.8.4.3(c)`. The labels are kept in this section because they are useful for reading the historical examples.)
...
This taxonomy is still partly empirical. Its purpose here is organizational: it tells the reader what the later synthesis must explain. (The synthesis in fact dissolves the regular/irregular division into a continuous digit-density statistic of the anchor; see `11.8.4.3(c)`. The taxonomy is retained as the historical map.)
...
should now be regarded as refuted. The later logarithmic identification of the anchor (`11.8.3.6`) shows tower type is a property of the `2`-adic digits of `-log ω / log 9`, and a computational audit over `2,499` families found no mod-`3` dependence in those digits at all — while also dissolving the regular/irregular division itself into a continuous digit-density statistic; see `11.8.4.3(c)` and Appendix `A.4.6`.
...
**Interpretation.**
The primary branch is therefore not a special empirical artifact of the irregular families. It is the canonical lifting-branch congruence object attached to every parameter `ω ≡ 1 (mod 8)` — whether that parameter is a valid family on the even component, or the companion `ω̃ = 3ω` of a family `ω ≡ 3 (mod 8)` on the odd component. The family-dependent datum is the compatible branch itself.
...
This is the formal endpoint of Stage 1 on the lifting branch. Section `11.8.4` separates what this synthesis has genuinely reduced from what remains open.
...
These should no longer be regarded as disconnected observations. The formal results of `11.8.3` now explain a substantial part of what these computations were pointing toward. What remains empirical is not the existence of the anchor mechanism itself, but the extension of the local anchor-displacement picture into a global one. In particular, the regular/irregular division in the first two items is now understood as a digit-density artifact of the anchor rather than a structural dichotomy; see `11.8.4.3(c)`.
...
**(c) Tower-type classification: dissolved, and the mod-`3` classifier refuted.**
Earlier drafts kept "globally classify regular versus irregular tower types" as an open question, with a proposed classifier by the residue of `ω (mod 3)`. Both parts are now settled, negatively, by the logarithmic identification together with a computational audit.

*Dissolution of the dichotomy.* Since `s = 3 + v_2(n - N(ω))` exactly, the `s`-profile of every lifting family is one universal profile translated by `N(ω)`. The only family-dependent datum is the digit expansion of `N(ω) = -log ω / log 9`; moreover `n_(k+1) = n_k` if and only if digit `k - 3` of `N(ω)` is `0`, so the visible refinement behavior of a tower is read directly off the anchor digits. The seven historically labeled families separate exactly by digit density: in the first `24` anchor digits, the regular families carry `14–15` ones (`ω = 25, 49, 73`) and the irregular families `8–12` (`ω = 17, 41, 65, 89`). Under the expected fair-coin digit model, that density is a `Binomial(24, 1/2)` draw, and the two "types" are simply the two tails of one unimodal distribution. There is no dichotomy to classify — only a continuous digit-density statistic, on which the historical sample of seven happened to straddle both tails.

*Refutation of the mod-`3` classifier.* An audit over all valid even-component families `ω ≡ 1 (mod 8)`, `3 ∤ ω`, `ω < 30000` (`2,499` families; anchor digits computed at precision `2^36` from the congruence definition, statistics on the first `24` digits) found no dependence of the anchor digits on `ω (mod 3)`:
...
The classifier is therefore removed entirely, not merely demoted; Appendix `A.4.6` retains the historical observation together with this negative result so that the same seven families do not regenerate the conjecture.

What survives here is only the anchor-digit question already recorded in `11.8.5.5`: the digit behavior of `2`-adic logarithms of small integers, a classical hard problem independent of the present program.
...
This criterion is exact and readable directly from the spike level, without computing `s` at the depth. It applies uniformly to all lifting-branch families, regular and irregular, and — via Corollary `11.8.5.2.2`, with spike depths `d_* = 2n_* + 1` and `v_2(n_* - N(3ω)) = k - 3` — equally on the odd component.

#### 11.8.5.5. Scope and the remaining open question

Theorem `11.8.5.2.1` and Corollary `11.8.5.2.2` together cover both components of the lifting branch as defined in `11.8.1.8.2`: odd `ω ≡ 1 (mod 8)` with `d` even, and odd `ω ≡ 3 (mod 8)` with `d` odd. The remaining residue-parity classes of Proposition `11.8.1.4.1` — namely `(ω ≡ 1, d odd)`, `(ω ≡ 5, any d)`, `(ω ≡ 7, any d)`, and `(ω ≡ 3, d even)` — have `s` pinned at `1` or `2` at the first layer; for these, the parity of `s`, and hence `3`-gain, is decided outright by the mod `8` classification, and no anchor machinery is needed. Within the lifting branch, the distinction between regular and irregular tower types does not affect the `3`-gain criterion itself.
...
so the remaining open question is precisely: describe the `2`-adic digit patterns of logarithms of small integers. This calibrates expectations. Digit questions about `p`-adic logarithms of integers belong to transcendence theory, are expected to be genuinely hard, and the digits themselves are expected to behave pseudo-randomly. The anchor formalism should therefore be valued for the reduction it achieves — the entire `3`-gain pattern of a family is compressed into one classical `2`-adic number, computable to any precision by a convergent series — rather than for the prospect of a closed-form description of that number's digits.

The `3`-gain law is therefore closed as a formal theorem on the entire lifting branch. What remains open on the anchor side is expected to remain open: any further "arithmetic organization" of the digits of `N(ω)` would constitute progress on the digit behavior of `2`-adic logarithms, a problem independent of, and older than, the present program. (The formerly hypothesized regular/irregular dichotomy and its mod-`3` classifier have been dissolved and refuted respectively; see `11.8.4.3(c)`.)
...
Under a fair-coin digit model this is a `Binomial(24, 1/2)` draw: the "types" are the two tails of one unimodal distribution, not two classes. Second, a computational audit over all `2,499` valid families `ω ≡ 1 (mod 8)`, `3 ∤ ω`, `ω < 30000` found no dependence of any tested anchor-digit statistic on `ω (mod 3)` (all effects below `1` standard error; largest per-digit frequency gap `≈ 1.8` se across `24` positions). The four-versus-three split observed above was a coincidence of a seven-family sample.
...
inside the valid lifting branch `ω ≡ 1 (mod 8)`. (This association has since been refuted: an audit of `2,499` families found no mod-`3` dependence in the anchor digits, and the regular/irregular distinction itself dissolves into a continuous digit-density statistic; see `11.8.4.3(c)` and `A.4.6`.)
