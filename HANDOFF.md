...
1. **The anchor question — author's active focus (2026-07-11).** Ben is turning attention to "pinning down the anchors." Framing settled this session: the anchor `N(ω)` is the **2-adic logarithm of the odd core** — the unique `N(ω) ∈ Z₂` with `9^N(ω) ≡ ω^(−1) (mod 2^k)` for all `k` (stage1.md 11.8.3.6), and the global law `s = 3 + v₂(n − N(ω))` says one block is governed by the 2-adic displacement from it. "Pinning it down" splits into two genuinely different questions:
