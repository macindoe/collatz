...
1. **Eric Merle collaboration — current state.**
   ...
   **Repos and ledger facts.** Shared repo `github.com/macindoe/one-obstruction-three-faces` (`PROTOCOL.md`, `LEDGER.md`, and — per his round-7 letter — a seeded `NOTE.md` skeleton; the letter says the repo is now public per PROTOCOL.md §7, but it was **not publicly accessible** at the round-7 check, 2026-07-23 — flagged, not adjudicated; `briefs/merle-round7-check-findings.md`). His handle `ericmerle3789`; his Lean repo `ericmerle3789/one-obstruction-three-faces-lean` (Lean 4 / Mathlib v4.27.0, kernel-verified L-A1 artifact with an honest scope header). Ledger entries: L1–L3 — the seed entries (L1 recorded "corrected, both directions"); L4 — his AEH class-skeleton replication, our key turned (aeh.md 13.4); L-A1 — the transport recurrence (cycles.md 12.6.1.1), both keys plus the kernel; L-A2 — the repeated-word gcd law (his proposal, both sides' keys; his round-7 letter adds a clean-room re-derivation on his side, 2400/2400 exact, canary green — entry unverified, repo inaccessible at check); L-A3 — the anchored-loops/spent-stock/Benford entry (cycles.md 12.6.1.2; his round-7 letter says co-edited on the shared side — unverified, same block).
   ...
   **Pending.** Round 7 received (pasted verbatim, 2026-07-23; closural register). Of round 6's two waits: the sibling-page link landed — `https://collatz-lab.org/cycles/` ("Two towers, four harbours") is live, back-links `https://macindoe.github.io/collatz/viz/cycle_anchor_gateway.html` (verified resolving to our gateway), and cites the v2 and mirror DOIs digit-exact; his LEDGER.md edits did **not** verify — the shared repo was not publicly accessible at check time (see Repos-and-ledger facts; flagged, per `briefs/merle-round7-check-findings.md`). Owed our side: (a) fill the footer cross-link slot in `viz/cycle_anchor_gateway.html` with his URL and push (separate delegate); (b) pin the NOTE.md realization-height reference in the shared repo — target determined our-side as itinerary.md 14.15.9(c) (Theorem 14.15.9.6 / Corollary 14.15.9.7; caveat in findings: re-confirm against NOTE.md's verbatim reference text once the repo is readable, since the alternative reading would pin 14.15.4(c)) — push only on the author's explicit go-ahead (separate delegate); (c) reply, drafted after (a)–(b) settle; sending stays with the author. At next repo access, verify the round-7 claims still on file unchecked: PROTOCOL.md §7, L-A2 (his 2400/2400 clean-room re-derivation), L-A3 co-edits, NOTE.md skeleton.
