...
where, in rotation order starting at $r$, $M_t = \sum_{j>t} m_j$ and $S_t = \sum_{j<t}\sigma_j$, with $\sigma_j = s_j + m_{j+1}$, indices cyclic (the step's $\sigma = \vt{C}$, $m_+ = \sigma - s$, of Definition~\ref{def:reduced}; the shift is essential, $m_{j+1}$ and not $m_j$). Hence $q \mid R_r$ and $q \le \min_r R_r$. For the trivial cycle, $R = 4^p - 3^p = q$ identically.
\end{proposition}
