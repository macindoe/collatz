...
v1, July 2026 (original publication). v2, July 2026: restored the subtitle of the \texttt{merle} citation; added a note evidencing the sharpness hedge of Theorem~\ref{thm:staircase} (Section~\ref{sec:cycles}), prompted by correspondence with Eric Merle. No theorem or universal claim is strengthened in v2, which adds a finite computational evidence record. v2-specific Zenodo DOI: 10.5281/zenodo.21421120. v3, August 2026: a correction appended to the Note added in v2 (Section~\ref{sec:cycles}) --- the note's identification of the remaining gap has been superseded on both halves in the project record, and the correction states what closed, at what scope, where the record is, and that the proof is established there and not reproduced here. The original sentence stays in place and Theorem~\ref{thm:staircase}'s statement is unchanged; its sharpness assessment, previously the theorem's closing clause, is now set out beside it under \emph{Sharpness evidence and assessment}, so that the theorem environment carries only what the paper proves. v3 also makes eight repairs, each bringing a definition or a statement back into line with the project record. Definition~\ref{def:reduced} now requires $\w > 0$, without which $(-1,1)$ leaves $F$ undefined; Theorem~\ref{thm:vlaw}'s unconditional bound reads $C(\w)(1+\log d)^2$, the printed $(\log d)^2$ being vacuous at $d = 1$; Theorem~\ref{thm:onestep} states its depth-$k$ window as the residues of Theorem~\ref{thm:deltaM} \emph{together with} the stratum labels $(s,\sigma,a_+)$, matching its own proof, with Theorem~\ref{thm:deltaM} unchanged; the depth component of $\pi_k$ (Section~\ref{sec:aeh}) is the exact convolution law rather than the window chain's stationary law; Hypothesis~\ref{hyp:aeh} is restated in ensemble form --- uniformly sampled starts, a horizon linked to the sampling scale, per-visit weighting, a single limit --- the previous single-orbit reading being empty on every convergent orbit; Theorem~\ref{thm:uniform}'s $n_0(p)$ is defined; the digit budget is labelled a heuristic, as its own text always said, and no part of it is now claimed as proved; and Proposition~\ref{prop:elim} defines $\sigma_j$ at its point of use ($\sigma_j = s_j + m_{j+1}$, as in Definition~\ref{def:reduced}) and distinguishes $M_t$ from the anchor. Section~\ref{sec:aeh} no longer states a descent or contraction consequence of Hypothesis~\ref{hyp:aeh}: that conclusion is a theorem without the hypothesis, and stronger, so the section is reframed onto what the hypothesis alone supplies, which is distributional. The stationary $3$-adic law it uses is attributed to Tao's Syracuse random variable, and the unconditional density line the hypothesis does not add to --- Terras, Everett, Korec, Inselmann, Tao --- is cited in Related work, together with the two further $3$-adic studies (Wirsching, Thomas) that a reader of Tao's footnote will reach. Verification pointers and script names are made concrete throughout. No numbered theorem's claim is strengthened, weakened, or renumbered, and nothing new is proved here; v3 reports a stronger result established in the project record and not reproduced in this paper. v3-specific Zenodo DOI: 10.5281/zenodo.21730505.
...
$n \le (\tfrac14 - \varepsilon)L$. Since $S$ accumulates at mean rate $4$ per block ---
...
on the far side of it. That frontier is the classical one: the same $\theta < 1/4$
is where Terras's count stops, and $1 - \beta/4 = \log_4 3$ is exactly the exponent
Korec \cite{korec} obtains by exhausting it. It is a frontier of this technique and
...
AEH implies the ledger with error $O(2^{-k})$ via Theorem~\ref{thm:onestep}, in the
form the hypothesis has: for every $\varepsilon$ and every horizon rate, all but a
set of starting values of natural density zero carry those frequencies along their
first $\lceil\theta\log_2 x\rceil$ bulk blocks. The descent consequence is not stated
...
$j$, the exact $\tfrac13$ rate of Lemma~\ref{lem:absorption}, and the depth law, at
every finite depth and past the budget at which the classical count stops. Its
exceptional set is a density-zero set of \emph{starting values} at a prescribed
finite horizon --- not a null set of orbits, and not a statement about any orbit's
infinite tail. It does not exclude individual staircase tails
