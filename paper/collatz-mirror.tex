...
using $2^{-M_3(y)}y^{-1} = -1$; the prefactor is a $3$-adic unit. For even $t \in \E$, lifting-the-exponent gives the mirror isometry $\vth{2^t - 1} = 1 + \vth t$ (from $\vth{4 - 1} = 1$ and the standard LTE step, valid in the inverse limit coordinatewise). Apply it to the bracket.
\end{proof}
...
\begin{proof}
$d = j$ iff $\vth{s - M_3(y)} = j - 1$, which cuts an exact residue condition mod $3^j$ on the admissible arithmetic progression, of density $(3^{j-1} - 3^{j-2}\cdot\ldots) $ --- directly, $2\cdot 3^{-j}$, since among $s$ in a full period mod $2\cdot3^{j}$ exactly $2\cdot3^{j-1} - 2\cdot3^{j-2}\cdot 3 = 2(3^{j-1}-3^{j-2})\cdot\ldots$; equivalently: $\vth{t} = j-1$ has density $3^{-(j-1)} - 3^{-j} = 2\cdot3^{-j}\cdot\tfrac{3}{2}\cdot\tfrac{2}{3} = 2\cdot 3^{-j}$ among all $t$, and the admissible $s$ sweep all residues.
\end{proof}
...
Classically, one third of odd integers are unreachable values. In reduced coordinates the phenomenon concentrates: states of depth $\ge 2$ merely lose one door on the mortal classes, and total barrenness occurs only at depth one. (A mortality-corrected branching heuristic for the full tree, with the measured stationary depth law as its one non-rigorous input, gives a renewal mass exceeding $1$ at every exponent below $1$, consistent with exact enumeration of the tree to $\w \le 2^{19}$; we do not use this heuristic anywhere below and flag it as such. The rigorous extraction is Section~\ref{sec:density}.)
...
Every kept child satisfies $y' > y$, except that the root's children satisfy $y' \ge 5$. Each node value determines its generating edge: $y'$ determines its state (if $y' \equiv 1 \bmod 3$ the state is $((y'+1)/2, 1)$; if $y' \equiv 2$, the state is recovered from $\vth{(y'+1)/2}$), the state determines the parent door $y = x_{\mathrm{exit}}$ of that state, and then $s$ is determined by $2^{s+1}y = 3y' + 1$. Consequently all node values in the door tree are pairwise distinct integers.
\end{lemma}
...
\begin{proof}
Nodes of the door tree are doors of states backward-reachable from $(1,1)$; by the equivalence theorem of \cite{paper1}, each node's $T$-orbit reaches $1$. Nodes are pairwise distinct odd integers (Lemma~\ref{lem:distinct}). Apply Lemma~\ref{lem:renewal} at the root: $\tilde\pi(X) \ge f(1, X) \ge 2^{-3.6}X^{0.3}$.
\end{proof}
