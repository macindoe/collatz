...
**Remark (relation to the trivial cycle).** The fixed point of the reduced dynamics is `(ω, d) = (1, 1)` with `M(1) = 0`. In anchor coordinates the convergence question becomes: do orbits drive the pair `(d - M(ω), M(ω))` to `(1, 0)`? This reformulation is recorded only to fix language; nothing is claimed.
