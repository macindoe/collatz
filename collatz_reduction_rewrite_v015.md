...
## 9.5. Upstream Predecessor Families of a Fixed Target

A different exact construction appears when one fixes a downstream odd target `x` and studies upstream predecessors of the form
...
This is an upstream construction, not a reduced-state representative family: in general the values `x_L` do not lie in a single reduced-state class. The parameter `L` records the power of `2` appearing in

```text
3x_L + 1 = x 2^L,
```

and should not be confused with the reduced-state valuation

```text
s_L = v_2(3^(d_L) ω_L - 1),
```

attached to the reduced state of `x_L`.
...
For admissible predecessors, one has the identity

```text
3x_L + 1 = x 2^L.
```

This shows that `L` measures the full `2`-adic depth of the predecessor relation to the fixed target `x`.

To relate this to the reduced structural quantity `s`, one must pass to the reduced state of `x_L`. The clean exact law occurs in the non-boundary terminal regime, namely when the predecessor `x_L` enters its own reduced block at terminal depth

```text
m_L = 1.
```

In that case, if `(ω_L,d_L)` is the reduced state associated to `x_L`, then

```text
A_L = 3^(d_L) ω_L - 1 = (3x_L + 1)/2.
```

Using `3x_L + 1 = x 2^L`, this gives
...
This yields the following conditional structural law.
...
be an admissible upstream predecessor whose reduced state lies in the terminal regime `m_L = 1`. Then
...
Thus in this terminal regime, `L` and `s_L` are not the same quantity: `L` measures the full `2`-adic depth in `3x_L + 1 = x 2^L`, while `s_L` measures the valuation after the mandatory first division by `2` in the structural numerator

```text
A_L = (3x_L + 1)/2.
```

It follows immediately that distinct admissible values of `L` in this regime cannot remain in a single reduced-state representative family, because within a fixed reduced-state family the quantity

```text
A = 3^d ω - 1
```

is constant, whereas here
