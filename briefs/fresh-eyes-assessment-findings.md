...
Out of scope on this branch: `sources/`, `paper/`, `publication.md`, `aeh.md`. The one question that touches the AEH verdict — whether Tao's *proof* delivers the letter statistics past the digit budget at logarithmic density — was handed to a parallel read (`briefs/tao-section5-read-findings.md`, branch `tao-section5-read`, commit `0ba527a`; not on `main` at the time of writing); its verdict is quoted in §3.3 and nowhere prejudged.
