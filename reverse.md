...
**Accounting** (extends `14.14.6`/`14.14.8`/`14.15.2`/`14.15.4`(d)/`14.15.5`(d)/`14.15.6`(e)/`14.15.7`(d)'s own accounting sentence). The whole single-letter periodic shelf is now closed by theorems: on every periodic single-letter word, height growth is a theorem — escape at the CRT-modulus rate `Q_n = 2^{(m+r)n+1}·3^{mn}`, with a constant that is either the fixed point itself (integer case, `14.15.7`, one sector capped at `|y^*|`) or a purely periodic, denominator-bounded unit-orbit value (non-integral case, this section, both sectors escaping with `v_n ∈ [1/q, 2)`). These are exact laws on the **decided** shelf: for such words the two adelic limits are pinned by the single rational `y^*`, and realization or escape is elementary arithmetic — the unit-group orbit of `Q_n mod q` and the parity of the exponent in `2^{2(m+r)n+1} mod 3`. What this does **not** do: bound `H` on any multi-letter or aperiodic word; move the Bridge (bridge.md §16) — the boundedness question for general words is exactly as open as `14.15.4`(d) left it, and the rigidity content of height growth now provably begins beyond periodic single-letter words. Nothing here reduces how much of the stratum word an infinite orbit requires (`11.8.7.7`, `14.14.6`, `14.15.2`).
...
## 14.15.9. The periodic shelf closed: whole-period height laws for every periodic word

*(Added 2026-07-17, branch `periodic-shelf`, per `briefs/periodic-shelf-brief.md`. `14.15.1`–`14.15.8` are merged and closed and are not edited here; this is a new subsection extending `14.15`'s numbering, per house norm. Provenance: the author-authorized multi-letter probe `briefs/multiletter-h-probe-findings.md` (merged 2026-07-17) confirmed the composed-unit mechanism on 13 period-2/3 words (948 verified rows), confirmed the rotation structure, found a cross-word spectrum coincidence, and refuted one simplification — the single-letter sharp constant `j_min/q` fails on 6 of its 13 words, the corrected constant being a minimum over the orbit of `j/q + k(t(j))`, and that corrected form is the one used throughout below, from the start. Its one permitted proposal — prove the identities in whole-period generality, the `14.15.8` lane's method applied once at the composed level — was author-authorized 2026-07-17; this section executes it. After this section the entire periodic shelf at whole-period windows is theorem-grade: every periodic word, any period, both sectors, integer or non-integral fixed point. The Bridge (bridge.md §16) is unmoved throughout; the rigidity content of height growth is hereby confined, by theorem, to aperiodic words. Register flat throughout.)*
