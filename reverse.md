...
**Proof.** Passing from `W_{(-n:0)}` to `W_{(-(n+1):0)}` prepends `(m_{-(n+1)},r_{-(n+1)})`; writing `α,β` for that letter's own affine constants (`14.14.4.1`), substituting `G(y_{-(n+1)}) = αy_{-(n+1)}+β` into `G^n(\cdot) = A_n(\cdot)+B_n` gives `A_{n+1}=A_nα`, `B_{n+1}=A_nβ+B_n`. So `B_{n+1}-B_n = A_nβ`, and `v_3(B_{n+1}-B_n) = v_3(A_n)+v_3(β) = M_n + 0` (`v_3(A_n)=M_n` by `14.14.8.2`; `v_3(β)=0` exactly as in `14.14.4.1`'s own proof, since `m_{-(n+1)} ≥ 1`). Each `m_i ≥ 1` gives `M_n ≥ n → ∞`, so `(B_n)` is Cauchy in the complete metric space `Z_3` and converges. ∎
