...
What the theorem does not close is the arithmetic of the anchor for the irregular families. For regular families, the anchor is approached through a clean nested congruence tower, and `v_2(ε)` at each depth is readily computable from the tower level. For irregular families, the halo structure around the spike sequence reflects the same underlying anchor, but the precise `2`-adic expansion of the anchor for those families carries structure that is not yet fully understood. This open question now includes the companion anchors `N(3ω)` for `ω ≡ 3 (mod 8)`, whose tower types (regular or irregular) should be catalogued alongside the even-component families.

The `3`-gain law is therefore closed as a formal theorem on the entire lifting branch. The remaining open question is an anchor-side one: whether the `2`-adic expansions of `N(ω)` and `N(3ω)` for irregular families have further arithmetic organization beyond what the spike sequence currently reveals.
