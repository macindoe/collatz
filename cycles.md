...
*Status of this lemma.* That a correctly-signed run exists at every scale is classical (continued-fraction density / the three-distance theorem), but a fully general, closed-form bound on the multiplicative gap between consecutive correctly-signed runs — needed to certify unconditionally that no period `p` is ever skipped, for `log_2 3` specifically — was not established in this session; this is recorded as a gap, not asserted away, and it is now the **sole remaining gap** of the floor-grade result below (see the `p = 22` resolution). What *is* established, scoped precisely: at `p = 15`, an exhaustive scan of essentially every integer `n` in a wide window found no passer regardless of Diophantine quality (`briefs/staircase-allp-findings.md` item 1) — there, the choice *among available* `n` was never the binding constraint. That finding does not generalize to every period: at `p = 22`, this lemma's own coverage gap — the multiplicative-gap bound left open above — bit in practice, and candidate *availability* itself was the binding constraint (resolved below, 2026-07-17).
...
**Proposition 12.8.6.4 (verified instance record, 2026-07-16; extended 2026-07-17).** By the recipe of `12.8.6.1`–`12.8.6.3`, exact big-integer verification (`experiments/staircase_allp.py`, fresh code, no import from `uniform_trim.py`) produces a configuration satisfying the size-passer condition (all `p` rotations, `q <= R_r`) and the scale condition (`n ~ 1.585^p` by construction) for every period

```text
p  ∈  {2, 3, ..., 23},        with     γ / log_2 p  ∈  [1.828, 3.643]
```

— now **contiguous** (the interval is unchanged from the 2026-07-16 record). The recipe's own output at `p = 7` (`n = 29`, `γ = 5.339`) is a *different* valid witness from `12.8.3`'s recorded instance (`n = 94`, `γ = 6.744`) — the staircase family is not unique at a given period, as `12.8.3`'s own `84` size-passers at `p = 6` already show. The published `p = 7` instance is separately reproduced and re-verified as a direct cross-check, independent of the general recipe: `m = (4,7,9,15,23,35,1)`, `s = (1,1,1,1,1,1,S-6)`, `γ = 6.744`, matching `12.8.3` to the stated precision, all seven rotations pass. Every constructed instance (recipe output and the cross-check) was checked against the full divisibility system `q | R_r`: none passes, matching `12.8.3`'s own finding for its instances; no halt condition was triggered — consistent with, but not a proof of, the assessed `γ = O(log p)` shape (`12.8.3`, `thm:staircase`).

**Extension (2026-07-17): the two `p = 22` rows.** Via two candidates outside `12.8.6.1`'s own chain (see the resolved Obstruction below), the same recipe (`12.8.6.2`–`12.8.6.3`, unmodified) produces the two size-passers that close the range above: `n = 25217` (`γ = 11.186`, `γ/log_2 22 = 2.508`, `13` correction moves) and `n = 31202` (`γ = 14.746`, `γ/log_2 22 = 3.307`, `8` correction moves) — both inside `[1.828, 3.643]`, both re-verified from `12.6.1` alone by `experiments/p22_passer.py`. The `13`-move correction is the largest move count in the record but stays within the same empirically-bounded-but-unproven picture already noted for the recipe generally (`briefs/staircase-allp-findings.md` item 5.3): no closed-form bound on the correction algorithm's move count is established, at `p = 22` or any other period.

**Obstruction (`p = 22`) — resolved (2026-07-17).** At `p = 22`, the bounded correction of `12.8.6.3` did not find a passing configuration within the tested budgets: up to `40` correction moves per candidate and `75` s of wall clock per period, six candidate `n` and two crash depths, in the committed script; up to `60` moves with no time limit, across a larger candidate set, in supplementary exploration recorded in `briefs/staircase-allp-findings.md`. `p = 24, 25` were also unresolved under the committed script's default budget but *did* resolve under the larger supplementary one (`8` moves each) — evidence that those two were a budget artifact rather than a structural wall, unlike `p = 22`, which resisted the larger budget too. That description is accurate as scoped and is kept for the record; what it left open — whether a different profile closes `p = 22`, or the obstruction is structural — is now answered.

*Resolution.* The failure was **candidate availability, not combinatorial resistance**. The chain the committed script actually supplies has a genuine hole at `p = 22`'s scale — `(16266, 31867)` under the script's own sign-agnostic candidate chain, or `(15601, 47468)` under lemma `12.8.6.1`'s strict sign-filtered grid (both intervals independently re-derived; `briefs/merle-pincer-check-findings.md` item 4) — and every in-scale `n` the chain did supply was either infeasible for the base construction or short by `16`–`19` bits, far beyond the correction algorithm's reach. At two candidates *outside* the chain — `n = 25217 = 15601 + 9616` and `n = 31202 = 2·15601`, simple arithmetic on the continued fraction's best convergent, named by Eric Merle in correspondence (his "Diophantine pincer" hypothesis) — Construction `12.8.6.2` plus Algorithm `12.8.6.3` resolve at the committed move cap: `13` and `8` correction moves respectively, well inside the `40`-move budget (extension above). Both certificates are independently verified from Proposition `12.6.1` alone by `experiments/p22_passer.py`.

Credit: Eric Merle's hypothesis correctly predicted this obstruction's *cause* (the Diophantine hole) and named the candidates that resolve it; his companion claim — that both points lie beyond the correction algorithm's empirical recovery threshold — is refuted at his own candidates (both resolve, in `13` and `8` moves). Symmetrically, this record's own prior claim, that the obstructions hit in this subsection are "combinatorial, not Diophantine" (`12.8.6.1`'s status paragraph, scoped there to the exhaustive `p = 15` scan), is refuted at `p = 22` in the complementary direction: there, candidate *availability* was exactly the binding constraint. The disagreement between the two records dissolves with both sides corrected — the first substantive product of the two-key correspondence protocol.

**Achieved grade: floor, substantially exceeded.** The brief's floor bar was verified instances to at least `p <= 20` with a documented obstruction; this reaches the full contiguous range `p ∈ {2, ..., 23}` via one general, per-period construction recipe plus a bounded, auditable (not exhaustive-search) correction algorithm — the one period (`p = 22`) the recipe initially could not close is now resolved (above), not merely documented. The primary theorem and both fallback gradations remain open: the all-`p` sharpness claim in `thm:staircase` is calibrated further — two isolated verified instances (`12.8.3`) become a contiguous, algorithmically generated range covering every period from `2` to `23` — but not proved; the sole remaining gap in this floor-grade result is now the Diophantine coverage bound of `12.8.6.1` (still open, per its status paragraph above). `12.8.5`'s strategic conclusion (the cycle front stays parked; the residual content of cycle exclusion is anchor-walk rigidity) is unchanged, as recorded above.
