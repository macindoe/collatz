...
##### 11.8.2.6.1. Provisional Theorem Target: existence, uniqueness, and compatibility of the primary branch

For each lifting family `ω ≡ 1 (mod 8)` and each level `k`, let `n_k(ω)` denote a solution class to
...
be the corresponding depth-side class.

The first theorem target is that these classes are not merely observed branches but are uniquely determined and compatible across levels. More precisely:

> **Provisional Theorem Target 11.8.2.6.1.**
> For each `ω ≡ 1 (mod 8)` and each `k >= 4`, there exists a unique congruence class
>
> ```text
> n_k(ω) (mod 2^(k-3))
> ```
...
> ```
>
> and these classes are compatible under reduction from level `k+1` to level `k`.

Equivalently, the depth classes

```text
d_k^*(ω) = 2 n_k(ω)
```

should form a unique compatible branch in the depth variable. Under this interpretation, the observed primary spikes are finite-level truncations of a single family-dependent `2`-adic anchor.

This theorem target is the arithmetic core of Stage `1B`. If proved, it would show that the primary branch is not a special empirical artifact of the irregular families, but the canonical lifting-branch object from which the irregular centered geometry is then measured.

##### 11.8.2.6.2. Provisional Theorem Target: universal centered halo law

Once the primary branch is isolated, the next target is to show that the remaining local geometry is universal after centering at it.

Empirically, if `d_k^*(ω)` is the primary spike for an irregular family and `Δ` is even with `Δ != 0`, then away from the spike itself one observes

```text
v_2(ω·3^(d_k^*(ω) + Δ) - 1) = 2 + v_2(Δ),
```

matching the transplanted `ω = 1` law.

This suggests the following theorem target.

> **Provisional Theorem Target 11.8.2.6.2.**
> Let `ω ≡ 1 (mod 8)` and let `d_*` be a branch point of exact spike height
>
> ```text
> H = v_2(ω·3^(d_*) - 1).
> ```
...
In this form, the theorem would say that off the spike, the valuation geometry depends only on the displacement from the branch point and not on the family `ω` itself. The family-specific information would therefore be exhausted by the anchor location and spike height, while the surrounding halo would be universal.

##### 11.8.2.6.3. Provisional Theorem Target: spike-height / jump-size relation

The centered halo law also suggests a structural explanation for the visible jump sizes of the primary branch.
...
then the local centered halo law forces any distinct competing refinement of the branch to occur at a displacement divisible by

```text
2^(H_k - 2).
```

In the visible irregular examples, the next refinement jump appears to occur at exactly that scale.

So the theorem target naturally splits into two parts:

* a structural divisibility barrier derived from the local halo law, and
* an empirical sharpness claim asserting that the barrier is attained.

In particular, one expects the following.

> **Provisional Theorem Target 11.8.2.6.3.**
> The exact spike height of the level-`k` primary branch determines the first dyadic scale at which the current truncation can refine; equivalently, branch jumps occur only at the scale predicted by the spike height.

If proved, this would explain why the observed irregular branch jumps are neither arbitrary nor family-by-family accidents, but are already encoded by the local valuation law around the primary anchor.

##### 11.8.2.6.4. Provisional Theorem Target: ratio-law encoding of relative branch offsets

A further structural consequence should govern relative branch placement. If
...
This suggests the following target.

> **Provisional Theorem Target 11.8.2.6.4.**
> For any two lifting families `ω_1, ω_2 ≡ 1 (mod 8)`, the relative offset
>
> ```text
> n_k(ω_2) - n_k(ω_1)
> ```
>
> is itself determined by the unique compatible solution class to
>
> ```text
> 9^m ≡ ω_1 · ω_2^(-1) (mod 2^k),
> ```
>
> and therefore the relative displacement of the depth-side anchors is governed by the same congruence mechanism as the anchors themselves.

The open problem here is not the formal congruence identity alone, but whether the compatible branch structure is uniquely determined by this relation and how that relative branch should be interpreted as a secondary `2`-adic anchor.

##### 11.8.2.6.5. Route A Stage 1B restated

With the preceding structure in place, the task of Route A Stage `1B` can now be stated more sharply:

> prove that in the irregular lifting families, the family-dependent data is exhausted by the compatible primary congruence branch, and then identify the arithmetic law by which that branch is encoded by `ω`.

Equivalently, Stage `1B` seeks to show that the irregular families contain no extra family-specific halo mechanism beyond the branch itself. Their apparent complexity should reduce to

```text
family-dependent 2-adic anchor
+ universal centered valuation law.
```

If this program succeeds, then the irregular halo ceases to be the main mystery. The real arithmetic content lies in the anchor: its existence, uniqueness, refinement law, and relative displacement between families.
