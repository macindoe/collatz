...
be an admissible upstream predecessor. If `S >= 1`, then `x_S` lies at the terminal interior position of its own reduced block. Hence its BlockEntry depth is

```text
m_S = 1.
```

**Proof.** By the phenomenology at the start of Section 9, every reduced block has deterministic interior
...
The case `S = 0` is the adjacent boundary case. Thus if `S >= 1`, the predecessor cannot lie at the boundary and must arise from the terminal end of a deterministic block. Therefore
...
Thus the structural cascade depth `S` is exactly the valuation of the predecessor structural numerator.

This family does not remain inside a single reduced-state representative class. Section 8 showed that within a fixed reduced-state family the quantity

```text
A = 3^d ω - 1
```

is constant across representatives, whereas here

```text
A_S = x_exit 2^S
```

varies strictly with `S`.

The running example `x_exit = 23` illustrates this exactly. Since `23 ≡ 2 mod 3`, only even `S` are admissible. For every admissible `S >= 1` one obtains
