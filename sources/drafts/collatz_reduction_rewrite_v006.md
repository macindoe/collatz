...
The fact that this induced transition depends only on the reduced state `(ω,d)`, and not on the particular BlockEntry representative chosen, is a substantive result and is proved later in Section 8.
...
Section 6 showed that an extended valuation block evolves deterministically up to its odd exit. The purpose of this section is to package that exact evolution into a single structural-step construction. This produces the bridge between BlockEntry coordinates and the reduced structural state formalism.
...
As noted in Section 5.6, `x_exit` is not another representative of the original reduced state `(ω,d)`, but the odd output of the structural step that initiates the next BlockEntry. That is, `x_exit` is part of the `(ω_+,d_+)` family.
...
## 9.5. What the Reduced Dynamics No Longer Need
...
The reduced map `F` is well-defined, but its effective structure is not yet classified. A central question is whether the transition
