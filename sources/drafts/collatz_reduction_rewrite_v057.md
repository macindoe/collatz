...
##### 11.8.2.6.7. Anchor-displacement reformulation of spike height

The additive-coordinate theorem of `11.8.2.6.6` shows that the family-dependent anchor is not merely an inverse-limit bookkeeping device. It is the natural exponent-side coordinate on the lifting branch
...
By Definition `11.8.2.6.5.1`, the global anchor `N(ω)` is characterized by the compatible congruences

```text
9^(N(ω)) ≡ ω^(-1) \pmod{2^k}
```

at every finite level. Since these congruences are compatible for all `k >= 4`, they assemble in `\mathbf{Z}_2` to the identity
...
This is the desired anchor-displacement reformulation.

**Proposition 11.8.2.6.7.1 (anchor-displacement form of the valuation law).**
Let `ω` be odd with
...
holds in `1 + 8\mathbf{Z}_2` by the defining property of `N(ω)`. Therefore, for `d = 2n`,
...
**Interpretation.**
This reformulation makes the role of the anchor completely explicit. The valuation is no longer best viewed as depending on two independent pieces of data, namely the family parameter `ω` and the exponent `n`. It depends only on the single displacement variable
...
In particular, the primary branch itself is now simply the zero-displacement locus

```text
n = N(ω),
```

viewed through its finite truncations modulo powers of `2`. The spike height at a chosen finite branch point is therefore the valuation of the corresponding anchor truncation error.
...
is the level-`k` primary branch truncation, then
...
So spike height is no longer an opaque family-specific measurement attached to a special residue class. It is exactly the `2`-adic order of the difference between a finite truncation and the global anchor.

This is the correct coordinate in which to reinterpret the centered halo law. Around a branch point, the natural local variable is no longer simply displacement from a chosen spike location in the depth coordinate. It is displacement from the global anchor itself.

Equivalently, the provisional picture

```text
family-dependent congruence branch
+ universal centered halo law
```
...
+ valuation law in anchor displacement ε = n - N(ω).
```

So the next Route A question is no longer merely whether the boundary-shell correction term admits further simplification in the local variables `(H,u,t)`. It is whether that term is already determined, or largely constrained, by the anchor-displacement variable

```text
ε = n - N(ω).
```

If so, then the apparent residual family dependence of the irregular branch would be reduced even further: not to the local spike unit `u`, but to anchor placement alone.

##### 11.8.2.6.8. Boundary-shell term in anchor-displacement form

The boundary-shell correction term of `11.8.2.6.3B` is not independent of the global
anchor coordinate `N(ω)`. It is exactly the valuation shadow of the normalized anchor
truncation error.

**Theorem 11.8.2.6.8.1 (anchor-displacement form of spike height).**
...
**Theorem 11.8.2.6.8.2 (boundary-shell valuation in anchor-error coordinates).**
...
**(a) Formal.** The following components are now part of the formal structure developed in Route A Stage 1:
...
- the sharpened boundary-shell localization theorem:
  ...
  `v_2(Δ) = H - 2`,
- the arithmetic normal form of the boundary shell via `B_H(u,t)`,
- the anchor-displacement reformulation of spike height,
- and the boundary-shell valuation law
  `v_2(B_H(u,t)) = v_2(q+t)`,
  showing that the boundary-shell correction term is not a new valuation invariant independent of the anchor.

Taken together, these results show that the family-dependent datum on the lifting branch is the additive anchor `N(ω)`, while the surrounding local valuation geometry is universal once expressed in displacement from that anchor. At the valuation level, the residual boundary-shell refinement is now also controlled by anchor displacement rather than by a second family-specific invariant.
...
- and the interpretation that the visible irregular family dependence is exhausted by anchor placement together with a universal off-spike valuation law.

Thus the irregular branch should no longer be regarded merely as a repository of unresolved exceptions. It already exhibits a coherent empirical geometry, and the formal results now explain a substantial part of that geometry rather than leaving it entirely heuristic.
...
- a full intrinsic identification of the boundary-shell correction term itself beyond its valuation,
...
formal:
    ...
    anchor-displacement form of spike height,
    boundary-shell valuation controlled by normalized anchor error
...
conjectural:
    ...
    intrinsic form of the boundary-shell correction term beyond valuation,
    full arithmetic classifier of tower type,
    global dependence of offset and spike profile on ω
```

This distinction is important for the discipline of Route A. The synthesis map is now stronger than a merely empirical guide: the branch mechanism is formal, the anchor mechanism is formal, the local halo mechanism is formal, and the boundary-shell valuation refinement is also formal. So the next task is no longer to determine whether a second valuation invariant survives beyond the anchor. The next task is to decide whether the remaining conjectural picture can be globalized and whether the correction term itself admits a cleaner intrinsic description beyond its valuation shadow.
