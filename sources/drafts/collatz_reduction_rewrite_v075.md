...
# 10. Empirical Observations: A Directory

The empirical layer of this note has been progressively either formalized or relocated. This short section records where each piece now lives, so that the main line of the note (Sections `9` and `11.8`) can be read without hunting.

* The family-by-family tower analysis, halo geometry, and the discovery path of the primary-branch congruence picture are in Appendix `A.4`–`A.5`; their formal replacements are the lifting-branch mechanism of `11.8.1` and the Stage 1 synthesis of `11.8.3`.
* The refuted mod-`3` tower-type conjecture, with the audit data that killed it, is preserved in Appendix `A.4.6`.
* The empirical digit statistics of the anchors (fair-coin behavior, density `0.497`) are in Appendix `A.4.6`, with their status calibrated in `11.8.4.2`.
* The frequency and size ledgers — the `s`-distribution along real orbits, the `3`-gain rate `1/3`, and the drift accounting — are in `11.8.4.4`.
* The numerical confirmations of the convergence translation are recorded with Section `9.8`.

What remains genuinely empirical after this reorganization is short: the anchor digit statistics and the orbit-level frequency ledger, both of which are conjecturally pseudo-random and both of which are now attached to precise formal statements rather than free-floating observations.
...
**Proof.** On the even component, apply Lemma `11.8.6.3.1` with `c = 1 - 2^s`, obtaining `s + m_+ = 3 + v_2(ε + δ_s)`, and subtract `s`. On the odd component, `3^d ω = 9^n·(3ω)` (Proposition `11.8.1.7.2`), so the same application with parameter `3ω ≡ 1 (mod 8)` gives the identical formula in `ε = n - N(3ω)`. Nondegeneracy: `ε + δ_s = 0` would force `3^d ω = 1 - 2^s` in `Z_2`; the left side is a positive integer, the right a negative integer, and distinct integers remain distinct in `Z_2`. ∎
...
If `s = 1` (the classes `(ω ≡ 1, d odd)`, `(ω ≡ 5, d odd)`, `(ω ≡ 7, d even)`, `(ω ≡ 3, d even)` of Proposition `11.8.1.4.1`), then
...
The framework developed in this note still does not resolve the Collatz conjecture. But it now states exactly what resolution means in its own terms (Section `9.8`): every `F`-orbit reaching `(1,1)`, with nontrivial cycles of the original map in bijection with nontrivial cycles of `F`. The status of the reduction has correspondingly changed: the main issue is no longer whether the reduced system is mathematically legitimate, but whether its arithmetic can be classified at a deeper level.
