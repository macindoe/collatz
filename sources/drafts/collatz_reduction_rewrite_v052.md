...
**Lemma 11.8.2.6.3B.4 (first normalized cancellation after the mod `4` jump).**
Let `d_*` be a spike point of height
...
```text
B_H(u,t) = u + α_H(t)(1 + 2^H u).
```

Equivalently,

```text
B_H(u,t) = (u + α_H(t)) + 2^H u α_H(t).
```
...
then `u + α_H(t)` is divisible by `4`, and one may write

```text
B_H(u,t)
=
4\left(
\frac{u + α_H(t)}{4} + 2^(H-2) u α_H(t)
\right).
```

Hence under the first cancellation condition,
...
Since `H >= 3`, the second summand above is always even, so this is equivalent to

```text
v_2(B_H(u,t)) >= 3
iff
\frac{u + α_H(t)}{4} is even
iff
u + α_H(t) ≡ 0 (mod 8).
```
...
From the definition of `α_H(t)`, one has

```text
3^(2^(H-2)t) = 1 + 2^H α_H(t).
```

Substituting this into the definition of `B_H(u,t)` gives
...
By Lemma `11.8.2.6.3B.3`, one has
...
Hence `(u + α_H(t))/4` is an integer, and from the second displayed form above one gets
...
This proves the normalized valuation formula.

Finally, because `H >= 3`, one has `H - 2 >= 1`, so

```text
2^(H-2) u α_H(t)
```

is even. Hence the quantity inside parentheses is even if and only if

```text
\frac{u + α_H(t)}{4}
```

is even. That is equivalent to
...
**Interpretation.**
This is the first real cancellation-mechanism lemma on the boundary shell. It shows that after the universal first jump

```text
v_2(B_H(u,t)) >= 1,
```

and after the first nontrivial matching condition

```text
u ≡ -t (mod 4),
```

the remaining problem is no longer to inspect `B_H(u,t)` in full. One may divide by `4` and reduce the next question to the simpler quantity

```text
\frac{u + α_H(t)}{4} + 2^(H-2) u α_H(t).
```

So the second higher-order jump is governed by whether
...
In particular, the boundary-shell problem is beginning to look like a genuine iterative cancellation mechanism rather than a diffuse collection of unrelated residues. The next question is whether `α_H(t)` itself admits a sufficiently rigid description modulo `8` to make this second criterion explicit.

**Lemma 11.8.2.6.3B.5 (the shell coefficient `α_H(t)` modulo `8`).**
Let `H >= 3`, let `t` be odd, and define
...
So to determine `α_H(t) mod 8`, it is enough to determine
...
We first determine `b_H mod 2^(H+3)`.
...
with coefficient `1 mod 8`.

Now suppose `H >= 4`. By Proposition `11.8.1.1.1` applied to `3^(2^(H-2)) - 1`, one has

```text id="o4v2m5"
v_2(b_H - 1) = H,
```

so there exists an odd integer `β_H` such that
...
For `H = 4`, this is immediate:

```text id="01t9rn"
β_4 = (3^4 - 1)/16 = (81 - 1)/16 = 5.
```

Now assume `H >= 4` and write
...
b_(H+1)
= (1 + 2^H β_H)^2
= 1 + 2^(H+1) β_H + 2^(2H) β_H^2.
```

Modulo `2^(H+4)`, the final term vanishes because for `H >= 4`,
...
```text id="in8j6r"
b_(H+1) ≡ 1 + 2^(H+1) β_H (mod 2^(H+4)).
```

So the new coefficient `β_(H+1)` satisfies

```text id="dah4qj"
β_(H+1) ≡ β_H (mod 8).
```

Since `β_4 = 5`, induction gives
...
```text id="jzjv7e"
b_H ≡ 1 + 5·2^H (mod 2^(H+3))
```

for all `H >= 4`.

We now raise to the odd power `t`.
...
Since any term of degree at least `2` in the binomial expansion is divisible by
...
Subtracting `1` and dividing by `8` yields

```text id="1o7fup"
α_3(t) ≡ t (mod 8).
```

Now suppose `H >= 4`. Then

```text id="b8exhk"
b_H^t = (1 + 5·2^H)^t
```

modulo `2^(H+3)`. Again, every binomial term of degree at least `2` is divisible by `2^(2H)`, hence by `2^(H+3)` because `H >= 4`. Therefore

```text id="9mjlwm"
b_H^t ≡ 1 + 5t·2^H (mod 2^(H+3)).
```

So

```text id="2a2gq4"
3^(2^(H-2)t) ≡ 1 + 5t·2^H (mod 2^(H+3)).
```

Subtracting `1` and dividing by `2^H` yields
...
**Interpretation.**
The shell coefficient `α_H(t)` continues to depend linearly on the odd shell coordinate `t`, but the coefficient changes once `H` passes from `3` to `4`:

```text id="c6mgp0"
α_H(t) ≡ t   (mod 8)   when H = 3,
α_H(t) ≡ 5t  (mod 8)   when H >= 4.
```

So the mod `8` behavior of the boundary shell is still highly rigid, but no longer completely uniform in spike height.

Combined with Lemma `11.8.2.6.3B.4`, this shows that the second higher-order jump on the boundary shell is governed by an explicit mod `8` matching rule between the normalized spike unit `u` and the shell coordinate `t`, with a single exceptional low-height case at `H = 3`.

**Corollary 11.8.2.6.3B.6 (explicit mod `8` criterion for the second boundary-shell jump).**
Let `d_*` be a spike point of height
...
and Lemma `11.8.2.6.3B.5` makes this condition explicit in terms of `t`.

**Proof.**

By Lemma `11.8.2.6.3B.4`,
...
So it remains only to substitute the mod `8` description of `α_H(t)` from Lemma `11.8.2.6.3B.5`.
...
**Interpretation.**
The second higher-order jump on the boundary shell is therefore governed by a completely explicit residue condition modulo `8`.

The first jump beyond spike height was universal:

```text id="yn0sop"
v_2(B_H(u,t)) >= 1.
```

The second jump required the mod `4` matching condition

```text id="x44v4r"
u ≡ -t (mod 4).
```

Now the third layer is also explicit:
...
So the boundary-shell cancellation is continuing to organize itself as a structured `2`-adic matching law between the normalized spike unit `u` and the odd shell coordinate `t`, rather than dissolving into unrelated residue phenomena. 

**Lemma 11.8.2.6.3B.7 (threshold valuation law for the boundary-shell correction term).**
Let `d_*` be a spike point of height
...
Equivalently, whenever `j ≠ H` one has

```text
v_2(B_H(u,t)) = min(j,H).
```

So the only genuinely new refinement occurs at the critical matching level
...
By Lemma `11.8.2.6.3B.4`, one has

```text
B_H(u,t) = (u + α_H(t)) + 2^H u α_H(t).
```

Since `u` and `α_H(t)` are odd, their product `u α_H(t)` is odd. Therefore
...
If `j < H`, then the two summands have distinct `2`-adic valuations, with the first smaller. Hence

```text
v_2(B_H(u,t)) = j.
```

If `j > H`, then the two summands again have distinct valuations, now with the second smaller. Hence

```text
v_2(B_H(u,t)) = H.
```

If `j = H`, then one may write

```text
u + α_H(t) = 2^H q
```

for some integer `q`, and therefore
...
because the two terms inside the parentheses are both odd when `j = H`: the quotient `(u + α_H(t))/2^H` is odd by definition of `j`, and `u α_H(t)` is odd. Hence their sum is even.

This proves the lemma. ∎

**Interpretation.**
The boundary-shell valuation is governed by a thresholded matching law.

Up to order `H-1`, the valuation of `B_H(u,t)` is exactly the `2`-adic order of the mismatch between `u` and `-α_H(t)`:

```text
v_2(B_H(u,t)) = v_2(u + α_H(t))
```

whenever this quantity is less than `H`.

If the matching exceeds order `H`, then the dormant correction term `2^H u α_H(t)` takes over and forces the valuation back down to `H`.

So the only point at which genuinely new higher-order cancellation can occur is the critical level

```text
v_2(u + α_H(t)) = H.
```

In this sense, the boundary-shell problem is not an unrestricted infinite residue chase. It is a thresholded `2`-adic matching problem with a single critical interface at height `H`.  

**Lemma 11.8.2.6.3B.8 (normalized critical-case reduction).**
Let `d_*` be a spike point of height
...
Assume the critical matching condition
...
Then `q_H(u,t)` is odd, and one has

```text
B_H(u,t) = 2^H ( q_H(u,t) + u α_H(t) ).
```

In particular, if one defines the normalized critical correction term by

```text
K_H(u,t) = q_H(u,t) + u α_H(t),
```
...
Under the critical matching condition
...
is an odd integer. Substituting this into the displayed identity gives

```text
B_H(u,t)
= 2^H q_H(u,t) + 2^H u α_H(t)
= 2^H( q_H(u,t) + u α_H(t) ).
```

This proves the factorization and the formula

```text
v_2(B_H(u,t)) = H + v_2(K_H(u,t)).
```

Now `u` and `α_H(t)` are both odd, so `u α_H(t)` is odd. Since `q_H(u,t)` is also odd, their sum
...
Finally, because `K_H(u,t)` is already even,
...
```text
q_H(u,t) + u α_H(t) ≡ 0 (mod 4).
```

Substituting back the definition of `q_H(u,t)` yields the final criterion. ∎

**Interpretation.**
The critical case is now reduced to a second normalized valuation problem.

Outside the critical level, Lemma `11.8.2.6.3B.7` shows that the boundary-shell valuation is simply

```text
v_2(B_H(u,t)) = min(v_2(u + α_H(t)), H).
```

So all genuinely new higher-order behavior is concentrated at the exact matching level

```text
v_2(u + α_H(t)) = H.
```

The present lemma shows that once this critical match occurs, the remaining refinement is governed by the new correction term

```text
K_H(u,t) = (u + α_H(t))/2^H + u α_H(t).
```

Thus the critical case does not destroy the cancellation mechanism. It reproduces it one level higher in normalized form: first there is an exact `H`-fold match between `u` and `-α_H(t)`, and then the residual valuation is determined by the parity and higher congruence behavior of `K_H(u,t)`.

So the next Route A question is no longer the full boundary-shell problem. It is the more focused problem of understanding the arithmetic of the normalized critical correction term `K_H(u,t)`.

**Lemma 11.8.2.6.3B.9 (critical-case reformulation in terms of `q_H(u,t)`).**
Let `d_*` be a spike point of height
...
Consequently, the normalized critical correction term
...
```text id="8hii1i"
K_H(u,t) = q_H(u,t)·3^(2^(H-2)t) - α_H(t)^2.
```
...
v_2(B_H(u,t))
=
H + v_2(q_H(u,t)·3^(2^(H-2)t) - α_H(t)^2).
```

**Proof.**

By the critical matching condition,

```text id="ewmqk5"
u + α_H(t) = 2^H q_H(u,t)
```

for some odd integer `q_H(u,t)`. Rearranging gives

```text id="g5ngmb"
u = -α_H(t) + 2^H q_H(u,t).
```

Now by Lemma `11.8.2.6.3B.8`,
...
```text id="jhu97g"
K_H(u,t)
= q_H(u,t) + (-α_H(t) + 2^H q_H(u,t)) α_H(t)
= q_H(u,t) + 2^H q_H(u,t) α_H(t) - α_H(t)^2
= q_H(u,t)(1 + 2^H α_H(t)) - α_H(t)^2.
```

Using the defining identity

```text id="xnfe2z"
1 + 2^H α_H(t) = 3^(2^(H-2)t),
```
...
Finally, Lemma `11.8.2.6.3B.8` gives

```text id="4bswzt"
v_2(B_H(u,t)) = H + v_2(K_H(u,t)),
```

so substituting the new form of `K_H(u,t)` proves the claim. ∎

**Interpretation.**
The critical case is now reduced to a new comparison problem:

```text id="9r7sff"
q_H(u,t)·3^(2^(H-2)t)  versus  α_H(t)^2.
```

So the normalized quotient `q_H(u,t)` is not merely an auxiliary definition. It is the precise family-specific datum that survives after an exact `H`-fold match between `u` and `-α_H(t)` has been factored out.

In particular, the critical layer no longer compares `u` directly to `-α_H(t)`. It compares the normalized residual quotient `q_H(u,t)` to the universal quantity `α_H(t)^2`, twisted by the same shell factor `3^(2^(H-2)t)`.

Thus the next question is whether `q_H(u,t)` has constrained residue behavior in actual primary spikes, or whether the critical case reveals a genuinely new family-dependent invariant beyond the anchor and first shell matching law.

**EDITOR NOTE**
The boundary-shell term is now in arithmetic normal form, but the remaining issue is not simply its residue behavior in (u,t); it is whether the normalized spike unit u itself is already determined by the anchor coordinate N(ω) and spike height H, so that no genuinely new family invariant survives on the boundary shell.

We will pause the line of investigation here in `11.8.2.6.3B.9` until we resolve the larger question.
