...
3. if
   ...
   ```text
   ω·3^(d_* + Δ) - 1 = 2^H (a + u·3^Δ),
   ```

   and therefore

   ```text
   v_2(ω·3^(d_* + Δ) - 1) = H + v_2(a + u·3^Δ).
   ```
...
with `a` odd. Substituting into the decomposition above gives
...
If `r >= 1`, then `2^r a` is even, while `u·3^Δ` is odd. So
...
with `a` odd and `u·3^Δ` odd. Hence
...
- and the sharpened boundary-shell localization theorem:
