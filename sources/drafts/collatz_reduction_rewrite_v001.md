# Collatz Reduction Rewrite

> **Review note:** Core notation has been stabilized around BlockEntry coordinates (u,m) and reduced states (ω,d). Remaining work is editorial only: final polish, optional examples/diagrams, and deciding whether any further transform remarks are useful.
...
The third layer records each block at its entry in canonical coordinates. Here u denotes the odd seed associated with the block entry and m records the local entry depth. These coordinates describe the block exactly and provide the natural input data for the structural step formula developed later.

### Layer 4. Reduced Structural States (ω,d)

The fourth layer passes from exact block coordinates to reduced structural states. Writing u = 3^a ω with 3 not dividing ω, the reduced state is obtained by retaining the structural odd core ω together with the full combined depth d = m + a. This is the genuinely reduced layer: different BlockEntry coordinates may determine the same reduced state, and the induced map on these reduced states is the main object studied in this note.

The first three layers should be viewed primarily as exact organization of the dynamics: they isolate deterministic structure, choose convenient coordinates, and prepare the quotient. The fourth layer is the novel step of the framework, where part of the arithmetic data is factored out and the dynamics are studied through the induced transition on reduced states.
...
The structural step on BlockEntry coordinates determines a corresponding transition on reduced states by passing through the projection Q. At this stage, the reduced map is introduced only at the level of construction:

- start from a BlockEntry (u,m),
- compute its structural-step output,
- project the result back to a reduced structural state.

The fact that this induced transition depends only on the reduced state (ω,d), and not on the particular BlockEntry representative chosen, is a substantive result and is proved later in Section 7.
...
- **F**: the induced reduced map obtained by passing the structural step through the projection Q.
...
- **x_exit**: the odd exit of the structural step, given by x_exit = A / 2^s.
...
This section collects the main coordinate transforms used throughout the note. They are not new dynamical laws; rather, they describe how the same object is expressed at different layers of the formalism.
...
Given a BlockEntry (u,m), write

u = 3^a ω,

where a = v_3(u) and 3 does not divide ω. Then define
...
This is exactly the projection Q introduced earlier.
...
Then the associated reduced state is

R(x) = (ω,d).

### 5.4. Transform R -> X_exit: Reduced State to Structural Exit

Given a reduced structural state (ω,d), define
...
Then x_exit is the odd exit of the structural step associated with (ω,d).

### 5.5. Transform X_exit -> R_+: Exit to Next Reduced State
...
Then the next reduced state is

R_+(x_exit) = (ω_+,d_+).

### 5.6. Combined Structural Pipeline

Combining the previous transforms gives the full reduced structural pipeline:

x -> (u,m) -> (ω,d) -> x_exit -> (u_+,m_+) -> (ω_+,d_+).

This separates coordinate changes from the structural dynamics themselves. The map F acts on reduced states, but its construction may be understood as passing through these intermediate transforms.

**Important distinction.** The value x_exit is not another representative of the reduced state (ω,d) that produced it. The representative family (u,m) = (3^r ω, d - r) consists only of BlockEntry descriptions of the same initial reduced state. By contrast, x_exit is the odd value obtained after the structural step has been completed, and it serves as the input for constructing the next BlockEntry and next reduced state. Thus the transforms X -> B -> R describe equivalent representations of a state, whereas R -> X_exit is a genuine dynamical transition.

## 6. Construction of the Reduced Structural Step

Section 5 showed that an extended valuation block evolves deterministically up to its odd exit. The purpose of this section is to package that exact evolution into a single structural-step construction. This produces the bridge between BlockEntry coordinates and the reduced structural state formalism.

### 6.1. From BlockEntry Data to the Structural Numerator
...
Applying Section 5 shows that the deterministic block evolves up to its endpoint according to the rigid drift formula, and that its odd exit is governed by the numerator
...
This quantity is the structural numerator attached to the BlockEntry (u,m), rewritten in reduced variables.
...
### 6.2. The Structural Step on BlockEntry Coordinates

Define the structural step E on BlockEntry coordinates as follows.

Given (u,m):

1. write u = 3^a ω with a = v_3(u) and 3 not dividing ω,
2. define the reduced structural depth

   d = m + a,

3. form the structural numerator
...
5. define the odd exit

   x_exit = A / 2^s,

   which is odd by construction, since s = v_2(A) removes the full 2-adic valuation of A,

6. define the next-entry depth

   m_+ = v_2(x_exit + 1),

7. define the next odd seed
...
Thus a full deterministic block is compressed into a single transition from one BlockEntry to the next.

### 6.3. Passage to Reduced Structural States
...
As noted in Section 5.6, x_exit is not another representative of the original reduced state (ω,d), but the odd output of the structural step that initiates the next BlockEntry.
...
The quantity s = v_2(A) measures the length of the 2-adic cascade at the block exit, while the resulting odd value x_exit determines the entry depth and odd seed of the next block. In this way, the internal drift proved in Section 5 is converted into a block-to-block transition rule expressed through the reduced variables (ω,d).
...
## 7. Well-definedness of the Reduced Map

Section 6 constructed the structural step on BlockEntry coordinates and showed how a corresponding reduced transition is obtained by projection. The purpose of this section is to prove that this reduced transition depends only on the reduced state (ω,d), and not on the particular BlockEntry representative chosen. In other words, the reduced map is well-defined.

### 7.1. Representative-Independence
...
### 7.2. Consequences for the Structural Step
...
are likewise uniquely determined. Writing

u_+ = 3^(a_+) ω_+,

with 3 not dividing ω_+, one then obtains

d_+ = m_+ + a_+.

Hence the projected output

Q(u_+,m_+) = (ω_+,d_+)

is uniquely determined by the original reduced state (ω,d).

### 7.3. Main Proposition

**Proposition 7.3.1.** If two BlockEntry coordinates satisfy
...
**Proof.** By Section 7.1, the reduced state (ω,d) uniquely determines the structural numerator A = 3^d ω - 1. By Section 7.2, A uniquely determines s, x_exit, m_+, u_+, and hence the projected output reduced state (ω_+,d_+). Therefore the result of applying E and then projecting by Q depends only on (ω,d), not on the representative chosen. This proves the claim.

### 7.4. The Induced Reduced Map
...
This is the sense in which the reduced structural state space supports its own intrinsic dynamics: the distinctions discarded by the projection Q do not affect the next reduced state.

### 7.5. Interpretation

Section 6 showed how to compress a full deterministic block into a single structural step. Section 7 shows that this compression is compatible with the reduction from exact BlockEntry coordinates to reduced structural states. Equivalently, all BlockEntry representatives of the same reduced state produce the same next reduced state.

For fixed (ω,d), all BlockEntry representatives lie on a single family

(u,m) = (3^r ω, d - r),

and every member of that family produces the same structural numerator A = 3^d ω - 1. This is why the reduced dynamics may be studied directly on the state space of pairs (ω,d), rather than only through the more detailed coordinates (u,m).

## 8. Proved Structural Properties
...
### 8.1. The Structural Numerator Governs the Step

For a reduced structural state (ω,d), the corresponding structural step is governed by the quantity

A = 3^d ω - 1.

By Sections 6 and 7, this numerator is determined entirely by the reduced state and is independent of the chosen BlockEntry representative. Its full 2-adic valuation
...
and hence determines the next-entry depth and next odd seed. Thus the reduced transition is controlled by the arithmetic of A.

### 8.2. 2-adic Proximity to -1

The next-entry depth is measured by the 2-adic proximity of x_exit to -1.

**Lemma 8.2.1.** For any odd integer y and any integer n >= 1,
...
### 8.3. Parity of s Controls 3-gain
...
This family serves as a useful reference model because the reduced step is governed by an explicit valuation law rather than by a general mixed reduced state.

### 8.5. What the Reduced Dynamics No Longer Need

For the purpose of computing the next reduced state, the separate values of the BlockEntry parameters a = v_3(u) and m do not matter once the reduced state (ω,d) is fixed. Section 7 showed that different representatives in the same family

(u,m) = (3^r ω, d - r)

all produce the same reduced output.

Accordingly, the reduced dynamics do not depend on the individual decomposition of d into a 3-adic contribution a and an entry-depth contribution m. What remains active is the reduced pair (ω,d) together with the valuation behavior of the associated structural numerator A = 3^d ω - 1.
...
### 9.1. Residue Clustering in the Reduced Dynamics
...
### 9.2. Apparent Locking of s-parity in Some Families

In some experimentally explored families, the parity of

s = v_2(3^d ω - 1)

appears to follow stable or repeating behavior over ranges of reduced states. Since Section 8 showed that the parity of s exactly controls whether the next stage gains a factor of 3, any such parity regularity would have direct dynamical consequences.
...
### 9.3. Near-Deterministic Average Behavior Within Some Residue Classes
...
### 9.4. Possible Finite-State Shadow of the Reduced Map

The reduced formalism naturally raises the question of whether the dynamics admit a finite-state shadow: that is, whether some bounded collection of residue variables captures a large part of the observable step-to-step structure, while the unbounded behavior is carried mainly by the depth variable d together with the valuations extracted from A = 3^d ω - 1.
...
### 9.5. The Role of the Approximate One-Third Phenomenon

Some computational patterns suggest that an approximate one-third-type behavior may emerge in families where the parity of s alternates or where 3-gain occurs with a visible regularity. The significance of this is that repeated 3-gain events do not simply mark local divisibility accidents: they repeatedly alter how much of the next step is absorbed into the reduced-coordinate decomposition, and so may influence longer-scale movement through the reduced state space.

At present, this is only a heuristic interpretation of numerical patterns, not a proved statistical law.

If such behavior is real, it would likely need to be explained through the interaction between the 2-adic cascade length s, the induced 3-gain mechanism of Section 8, and the residue structure of ω and d. Establishing or refuting such a mechanism remains future work.

### 9.6. Status of These Observations
...
### 10.1. Classification of Reduced-State Transitions

The reduced map F is well-defined, but its transition structure is not yet classified. A central question is whether the transition

(ω,d) ↦ (ω_+,d_+)

admits a cleaner direct description than the current step-by-step extraction through A, s, x_exit, and the next BlockEntry data.
...
- Is there a more intrinsic formulation of F that avoids passing back through the full BlockEntry reconstruction?

### 10.2. Structure of the Valuation s = v_2(3^d ω - 1)
...
### 10.3. Mechanisms of 3-gain

Section 8 proved that the parity of s determines whether the next odd seed gains a factor of 3. What remains open is to understand the broader structural role of repeated 3-gain events.
...
### 10.4. Minimality of the Reduced Coordinates

The current reduced state (ω,d) is natural for the reduced formalism developed here, but it remains open whether it is already minimal for describing the induced map.

More specifically:

- Is the pair (ω,d) already the most natural coordinate system for the reduced dynamics?
- Can ω itself be compressed further in some structured way?
- Is there a more intrinsic description of F in which the structural numerator A = 3^d ω - 1 appears even more directly?

### 10.5. Finite-State Shadow and Residue Control
...
### 10.6. The Approximate One-Third Phenomenon
...
### 10.7. Relationship to Classical Collatz Formulations
...
### 10.8. Closing Perspective

The framework developed in this note does not resolve the Collatz conjecture, but it does isolate a coherent reduced dynamical system that is defined explicitly. The main open problem is no longer whether the reduction is mathematically legitimate; that point has been established. The deeper challenge is to determine whether the reduced map F admits further compression, classification, or invariant structure beyond what is currently visible.
