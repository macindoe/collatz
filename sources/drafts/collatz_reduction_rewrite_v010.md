...
Suppose `x_L` is integral and satisfies
...
**Proposition 9.5.1** Let `x` be fixed, and let

```text
x_L = (x 2^L - 1) / 3
```

be an integral upstream predecessor satisfying `v_2(x_L + 1) = 1`. Then the associated structural numerator and its 2-adic valuation are
...
This gives an exact linear law for `s` along any stabilized upstream family of this form.

It also shows that such a family does not remain inside a single reduced-state representative class. Indeed, Section 7 showed that within any fixed reduced-state family the quantity
...
varies strictly with `L`. Thus distinct admissible values of `L` in the stabilized regime must correspond to distinct reduced-state families.

The running example `x = 23` illustrates this exactly. For odd admissible `L >= 3`, one obtains
...
The case `L = 1` is exceptional because the condition `v_2(x_L + 1) = 1` has not yet stabilized: here `x_1 = 15` and `v_2(x_1 + 1) = 4`.
