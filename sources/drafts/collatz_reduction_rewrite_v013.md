...
form the full deterministic interior of the same reduced block: the structural odd core `ω` remains fixed, the total depth `d = m + a` remains fixed, and only one unit of `2`-adic depth is exchanged for one unit of `3`-adic content at each step.

The terminal position of this interior chain occurs at

```text
(m,a) = (1,d-1),
```

where the next odd step is no longer an interior one-halving move within the same block. Instead, the block exits through the structural numerator
...
gives the forced exit cascade, and the odd exit is

```text
x_exit = (3^d ω - 1) / 2^s.
```

This exit data depends only on `(ω,d)`. Hence every representative of the same reduced block shares the same terminal cascade length `s` and the same odd exit `x_exit`.

In this sense, a reduced structural state `(ω,d)` packages an entire finite deterministic odd chain together with its common exit law. The reduced map does not track the interior positions individually; it compresses the whole block to the single datum needed to determine its eventual odd exit and the next reduced state.

````

And here is a slightly stronger version, if you want to explicitly distinguish the `s=1` boundary case from the genuine multi-halving cascade regime:
...
This construction is upstream rather than representative: the values `x_L` need not lie in the same reduced-state family, and in general they do not. Instead, they form a family of structural predecessors of the fixed target `x`, indexed by cascade depth.
...
is integral if and only if `3` does not divide `x` and the parity of `L` is determined by the residue of `x mod 3` as follows:
...
If `3` divides `x`, this is impossible. If `x ≡ 1 mod 3`, then one must have `2^L ≡ 1 mod 3`, which holds exactly when `L` is even. If `x ≡ 2 mod 3`, then one must have `2^L ≡ 2 mod 3`, which holds exactly when `L` is odd.

Now suppose `x` is odd with `3` not dividing `x`, and let `x_L` be an admissible predecessor.

The key structural point is that Lemma 6.2.1 governs deterministic drift within a block: each odd step transfers one unit from entry depth into `3`-adic content while preserving `d = m + a`. Thus a genuine cascade does not arise at an arbitrary interior point of a block, but only at its terminal end, after the available entry depth has been exhausted. This is why the boundary case `L = 1` may behave differently from longer cascades.
...
Let `(ω_L,d_L)` be the reduced state associated to `x_L`, and define
...
Thus any admissible odd target `x` gives an exact linear law for `s` along its non-boundary upstream cascade family.
...
varies strictly with `L`. Thus distinct admissible values of `L` in this cascade regime must correspond to distinct reduced-state families.

The running example `x = 23` illustrates this exactly. Since `23 ≡ 2 mod 3`, only odd `L` are admissible. For every admissible non-boundary cascade predecessor one obtains
...
The case `L = 1` is exceptional because it lies at the adjacent boundary of the same reduced-state family rather than in the stabilized cascade regime.
