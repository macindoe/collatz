...
Here the index `S` is the structural cascade depth attached to the predecessor family.

The adjacent boundary case corresponds to `S = 0` and is exceptional because it lies at the boundary of the same reduced-state family rather than in the stabilized cascade regime. This is apparent when considering

```text
x_0 = (2x-1)/3.
```

The full predecessor identity is
...
By Lemma 9.5.2, every admissible predecessor that is not the adjacent boundary predecessor lies in the terminal regime
...
be an admissible upstream predecessor in the fixed-target family of this section. If `x_S` is not the adjacent boundary predecessor (i.e. `S >= 1`), then it lies at the terminal interior position of its own reduced block. Hence its BlockEntry depth is

```text
m_S = 1.
```

**Proof.** By the phenomenology stated at the start of Section 9, every reduced block has a deterministic interior whose BlockEntry coordinates run through

```text
(m,a) = (d,0), (d-1,1), ..., (1,d-1),
```

with unique terminal interior position

```text
(m,a) = (1,d-1).
```

The same discussion distinguishes the boundary case `s = 0` from the genuine multi-halving cascade regime `s >= 1`. Thus an upstream predecessor which is not the adjacent boundary predecessor cannot occur at the boundary of the block. It must therefore arise from the terminal end of a deterministic block, i.e. from the terminal interior position. At that position the BlockEntry depth is exactly

```text
m_S = 1.
```

This is precisely the terminal regime used in the calculation below. ∎
