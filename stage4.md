...
2. **The cycle benchmark**: a nontrivial cycle is a closed anchor walk with `Σ ΔM_t = 0` (`9.8.4`, anchor form); the low-order law makes every cycle candidate satisfy an explicit finite congruence system per stratum sequence. Test whether this reproduces or sharpens the known cycle-length constraints (`11.8.3.11`). *(Opened: cycles.md §12. Period 1 is closed — the fixed-point equation reproduces Steiner's circuit theorem exactly (Theorem `12.2.3`); the congruence system is stated (`12.3`) and period 2 is the next concrete target.)*
