...
**Calibration (delay-line reading; 2026-09-04).** The consumption identity above is a statement about *unread* digits, and a bit-flip experiment fixes what it does and does not say. Flip one bit `j` of a `256`-bit starting core (`177` starts, `40` blocks, `k = 8`, `d_0 ≤ 4`): the letter word first moves at block `≈ j/4` (mean first-difference block `2.2` at `j = 8`, `4.4` at `16`, `8.3` at `32`, `16.4` at `64`, `32.0` at `128`; no bit `j ≥ 200` moves any letter within `40` blocks — `4,950` start–bit pairs — the cumulative `σ` at block `40` averaging `161`). The budget is a *delay line*: bit `j` is not consulted until the running `Σσ` reaches it. Once read, a bit is never spent: flipping any of bits `1..8` changes the core `ω_t mod 2^8` with probability `0.94`–`0.99` at every block `t = 1..40`, changes the exit integer `x_exit(t) mod 2^8` with probability `0.94`–`0.99`, and changes the letter `s_t` with probability `0.60`–`0.70` at every block from `2` on (`0.657` at `t = 16`, mean over bits `1..8`; `0.653` over bits `1..32`) — the value for two independent draws from the ledger, `1 − Σ_j 2^(−2j) = 2/3`. What "nothing regenerates" says, made exact for a bit flip, is locality and nothing more: the underlying integer `X_t = 3^(d_t) ω_t` agrees with its flipped copy modulo `2^(j − Σ_(i<t) σ_i)`, so the exit and the letter at block `t` cannot move once `j ≥ Σ_(i<t) σ_i + k + s_t` (zero violations of that sharp per-start bound in the run, for exits and for letters). What is not regenerated is *local* — residue-class — determinism, which is the classical statement (Terras's cylinder count; Lagarias's shift conjugacy; citations pinned in publication.md). The "regeneration" every later block visibly performs is the carry propagation of the multiplication by `3^d`, which scrambles the whole core (normalized Hamming distance `0.47`–`0.49` from block `3` on) without reading any new digit. One coordinate artifact, recorded so that it is not mistaken for a locality violation: the core `ω_t mod 2^k` *does* move under high-`j` flips at blocks `1`–`3` (`2,263` events with `j ≥ Σσ + k + s_t`, all at `t ≤ 3`, none later). Every one is a factor of `3` moved between `ω` and `d` at an earlier resonant step (stage3.md `11.8.6.2.1`, the case `h(s) = d`, where `a_+ = d + v_3(ω + β)` reads high bits of `ω`): in all `2,263` the exit, every letter `s_0..s_t`, and `X_t mod 2^k` are unchanged and `d_t` differs (by one to seven factors of `3` in the run). The exit integer and the letters obey strict locality; the core in `(ω, d)` coordinates does not, by construction. Reading: the budget bounds what a bounded window can *decide*, not how long the orbit remembers its start; the organizing heuristic above is unchanged, and no status moves. *Verified* — `experiments/digit_budget_avalanche.py`, fresh code (imports nothing from any other script; exact integer comparison at every pass/fail decision; seed `4`; re-running reproduces `experiments/digit_budget_avalanche_output.txt` byte for byte), 2026-09-04: `177` starts × `255` flips × `40` blocks, the three change matrices (core, exit, letter), the sharp locality bound, the resonant-step diagnosis — `4` checks, `0` failures.
