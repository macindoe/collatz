...
2. **The cycle benchmark**: a nontrivial cycle is a closed anchor walk with `Σ ΔM_t = 0` (`9.8.4`, anchor form); the low-order law makes every cycle candidate satisfy an explicit finite congruence system per stratum sequence. Test whether this reproduces or sharpens the known cycle-length constraints (`11.8.3.11`). *(Opened: cycles.md §12. Periods 1 and 2 are closed — the fixed-point equation reproduces Steiner's circuit theorem (Theorem `12.2.3`), and the two-step elimination reproduces the Simons–de Weger 2-cycle case with eleven divisibility tests (Theorem `12.5.3`). Period `p >= 3` via the same schema is the open front.)*
