...
So the remaining problem is not a diffuse global failure of the centered halo law. It is concentrated in one arithmetic correction term on that shell.
...
**Interpretation.**
The boundary shell is not merely the first shell on which off-spike excess may occur. It is a shell on which one always gains at least one additional factor of `2` beyond the spike height `H`.

So the remaining Route A question is no longer whether off-spike excess can occur. It can. The real question is whether the higher valuation of

```text
B_H(u,t)
```

is governed universally by the shell coordinate `t`, or whether the normalized spike unit `u` carries genuine residual family-specific data.

At the same time, the surrounding results of `11.8.2.6.4`–`11.8.2.6.6` suggest that the true family-dependent datum is not the local spike unit by itself, but the global anchor coordinate `N(ω)`. So the next task is to determine whether the boundary-shell term is genuinely new arithmetic, or merely a local shadow of anchor displacement.
...
##### 11.8.2.6.8. Route A Stage 1B restated
...
On the other hand, `11.8.2.6.2` and `11.8.2.6.3` show that once a branch point is fixed, the surrounding valuation geometry is universal away from the boundary shell. Thus the local halo is no longer the main mystery. The remaining issue is whether the residual boundary-shell correction term carries any genuinely new family-specific information beyond the anchor.

Accordingly, Route A Stage `1B` now splits into two explicit subproblems.

**Stage `1B(1)`: recast spike height in anchor-displacement form.**

By `11.8.2.6.7`, on the lifting branch `d = 2n` one has

```text
v_2(ω·3^d - 1) = v_2(9^(n - N(ω)) - 1).
```

So the spike-height problem is no longer best viewed as the valuation of a mixed expression in `(ω,d)`. It is the valuation of the anchor displacement

```text
ε = n - N(ω).
```

In particular, if `n_k(ω)` is the level-`k` primary branch truncation, then the corresponding spike height is

```text
H_k(ω) = v_2(9^(n_k(ω) - N(ω)) - 1).
```

Thus the first task of Stage `1B` is to reformulate spike height, and more generally local lifting size, entirely in terms of the displacement from the global anchor.

Equivalently, the first question is:

> can the visible spike geometry be read directly from the `2`-adic size of the truncation error
> `n_k(ω) - N(ω)`?

If so, then spike height is not an additional family-specific invariant. It is simply the valuation profile of finite approximation to the anchor.

**Stage `1B(2)`: decide whether the boundary-shell term carries anything beyond that coordinate.**

After `11.8.2.6.3B`, every possible failure of the universal centered halo law is concentrated on the single boundary shell

```text
v_2(Δ) = H - 2,
```

where the valuation is governed by

```text
v_2(ω·3^(d_* + Δ) - 1) = H + v_2(B_H(u,t)).
```

So once spike height has been reinterpreted through anchor displacement, the remaining question is whether the boundary-shell correction term

```text
B_H(u,t)
```

contains any genuinely new family-dependent datum.

More precisely, the second task of Stage `1B` is:

> determine whether the valuation of `B_H(u,t)` is already forced, or largely constrained, by the anchor-displacement coordinate together with the universal shell geometry, or whether the normalized spike unit `u` carries an additional invariant not reducible to anchor placement.

In this form, the logic of Stage `1B` is cleanly ordered:

1. first absorb the primary spike into the anchor coordinate `N(ω)` and rewrite spike height as a displacement valuation;
2. then test whether the residual boundary-shell arithmetic survives that change of coordinates as genuinely new structure.

So the provisional lifting picture is now:

```text
global additive anchor N(ω)
+ valuation in anchor displacement
+ possible residual boundary-shell correction.
```

The strategic aim of Stage `1B` is to determine whether the third term is real, or whether it too is only a local shadow of the first two.

If the boundary-shell term turns out to be controlled entirely by anchor displacement, then the apparent family-specific complexity of the irregular lifting branch would be exhausted by the single additive coordinate `N(ω)`. In that stronger form, the irregular branch would no longer consist of a family-dependent spike together with extra residual structure, but simply of universal valuation geometry expressed in the wrong coordinate system until recentered at the global anchor.

##### 11.8.2.6.9. Boundary-shell term in anchor-displacement form
...
**Theorem 11.8.2.6.9.1 (anchor-displacement form of spike height).**
...
**Theorem 11.8.2.6.9.2 (boundary-shell valuation in anchor-error coordinates).**
