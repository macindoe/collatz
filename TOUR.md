...
- The `thm:staircase` hedge — sharpness assessed, not proved, for all `p` — is exactly `cycles.md` 12.8.3 (a *Remark* there, deliberately). The attempt to close it is `cycles.md` **12.8.6**: a Diophantine lemma (semiconvergents of `log₂3`), an explicit profile, a bounded correction, verified instances for `p ∈ {2,…,21} ∪ {23}`, and a documented obstruction at `p = 22`. Grade and gaps are stated in the section itself.
- The diagnostic detail behind the `p = 22` obstruction, and the three untried construction variants, are in `briefs/staircase-allp-findings.md`. The delegation brief that scoped the attempt is `briefs/staircase-allp-brief.md`.
