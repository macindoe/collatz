---
status: CLOSED as a local law (15.1–15.2, verified, zero failures); the author's divergence question answered exactly (15.3); the composition with the step law worked out (15.6, the tear; 15.7, the two commutation laws)
...
This closes the vertical question as a local law. Open connections worth recording, not yet pursued: the ladder gives a second exact mechanism relating *different states with the same anchor* (at the valuation level identified in `15.5` as the target-shift lemma read in the depth variable), which is raw material for the fiber-to-orbit bridge (stage2.md `11.8.5.6`; consolidated as the Bridge, bridge.md §16) — the bridge asks how anchors vary along orbits, and the ladder shows how orbits vary along an anchor; the composition of the ladder with the step law itself is `15.6` (the tear) and `15.7` (the guaranteed neighbours: the off-spike merge/skip law, the two-row commutation, and the proof that these are the only two). The mirror front has its dual ladder: reverse.md `14.10` relates predecessors at adjacent branches `s` and `s+2` at a fixed door (step size `2` forced by the parity constraint), with an exact dichotomy gated by the `3`-adic anchor's first digit — the mirror of `15.1`'s tear-line.
...
**Remark 15.6.8 (off-spike).** The `s=1` case (the two vertical neighbours' merge/skip structure) is Lemma `15.7.1`; not stated or proved here.
...
(`294` cases), and exactly `0.000` on the strata `(2,3,3)` (`294` cases) and `(3,1,1)` (`5,101` cases). On those two strata the repeat is with the *grandchild* `F²(ω,d)`, not the child: the boundary case of `15.7.5` at `(2,3,6)` followed by law (i)'s merge in column `Ω`, and law (i)'s skip followed by law (ii) — grandchild-core rates `0.249` and `0.063` (review re-derivation, fresh code, `4,186` and `66,919` cases, 2026-09-13), compositions of the two laws and nothing new. One
